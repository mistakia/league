...
const generate_for_date_and_category = async ({
  ...
  if (missing.length === required_sources.length) {
    log(
      `WARN: no calibration rows for format_category=${format_category} date=${date_iso}; skipping`
    )
    return []
  }
  ...
  for (const pid of player_ids) {
    const k_native = latest_in_window_by_pid(
      ktc_idx,
      pid,
      date_iso,
      STALENESS.KTC
    )
    const a_native = latest_in_window_by_pid(
      adp_idx,
      pid,
      date_iso,
      STALENESS.ADP
    )
    const r_native = latest_in_window_by_pid(
      rank_idx,
      pid,
      date_iso,
      STALENESS.RANKINGS
    )
    const p_native = latest_in_window_by_pid(
      props_idx,
      pid,
      date_iso,
      STALENESS.PROPS
    ...
    const coverage =
      intended_player_weight > 0
        ? Math.min(1, weight_sum / intended_player_weight)
        : 0
...
const generate_composite_market_value = async ({
  start_date,
  end_date,
  rebuild = false
}) => {
    ...
    for (const m of mappings) {
      ...
      if (player_rows.length) {
        await batch_insert({
          items: player_rows,
          save: (items) =>
            db('composite_market_value_daily')
              .insert(items)
              .onConflict(
                db.raw(
                  '(format_category, player_id, date) WHERE asset_type = 1'
                )
              )
              .merge(),
          batch_size: 5000

...
for host in league league-worker-1; do
  ...
  ssh $host "source ~/.nvm/nvm.sh && nvm install $NODE_VERSION && ln -sfn ~/.nvm/versions/node/v$NODE_VERSION ~/.nvm/versions/node/current && echo 'Done: current -> v$NODE_VERSION'"
  echo ""
done

# Local machine (worker-2 / Mac)
echo "Setting up local (worker-2)..."
source ~/.nvm/nvm.sh && nvm install $NODE_VERSION && ln -sfn ~/.nvm/versions/node/v$NODE_VERSION ~/.nvm/versions/node/current
echo "Done: current -> v$NODE_VERSION"
echo ""

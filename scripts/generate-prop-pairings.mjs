...
const format_prop_pairing = ({ props, prop_stats, week, team }) => {
   ...
   *   (accumulator, prop) => ({
   *     risk_total: accumulator.risk_total + prop.risk,
   *     payout_total: accumulator.payout_total + prop.payout
   *   }),
   *   {
   *     risk_total: 0,
   *     payout_total: 0
...
const generate_prop_pairings = async ({
  ...
  // const first = prop_groups_data_by_edge.find(item => item.props.length === 1)
  // log(first)

...
const audit_player_gamelogs = async ({
  ...
  if (!pfr_player_gamelogs_for_season.length) {
    log(
      `WARNING: PFR returned 0 gamelogs for ${season_year} -- PFR may be blocked by Cloudflare or unreachable`
    )
    if (collector) {
      collector.add_warning(
        `PFR returned 0 gamelogs for ${season_year} -- validation skipped`,
        { season_year }
      )
    }
    return result
  ...
  for (const pfr_player_gamelog of pfr_player_gamelogs_for_season) {
    ...
    const player_gamelog = player_gamelogs.find(
      (p) =>
        ...
        p.season_type === pfr_player_gamelog.season_type
    )

    if (!player_gamelog) {
      result.missing_gamelogs++
      log(
        `missing gamelog for ${pfr_player_gamelog.pfr_id} week ${pfr_player_gamelog.week} ${pfr_player_gamelog.season_type}`
      )
      if (collector) {
        collector.add_warning(
          `Missing gamelog for ${pfr_player_gamelog.pfr_id} week ${pfr_player_gamelog.week}`,
          {
            ...
            season_type: pfr_player_gamelog.season_type
          }
        )
      }
      continue
    ...
    if (differences && differences.length) {
      log(
        `differences for ${player_gamelog.pid} week ${player_gamelog.week} ${player_gamelog.season_type} pfr_game_id ${pfr_player_gamelog.pfr_game_id}`
      )
      log(differences)
      result.discrepancies.push({
        ...
        season_type: player_gamelog.season_type,
        differences
      })

...
const generate_player_snaps_for_week = async ({
  ...
  for (const snap of nfl_snap_rows) {
    ...
    if (play_type === 'PASS' || play_type === 'RUSH') {
      ...
      if (score_diff > 3) team_totals[off].off.snaps_leading.add(play_key)
      if (score_diff < 3) team_totals[off].off.snaps_trailing.add(play_key)
    ...
    for (const play of player_snap_rows) {
      ...
      if (play.play_type === 'PASS' || play.play_type === 'RUSH') {
        if (play.off === player_gamelog.tm) {
          ...
          if (play.score_diff > 3) player_snaps.leading.add(play_key)
          if (play.score_diff < 3) player_snaps.trailing.add(play_key)

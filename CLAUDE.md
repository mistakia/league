...
| Touch a data-view or plays-view column, param, field factory or golden | [docs/guides/data.md](docs/guides/data.md)           |
...
- **A claim a GUIDE hands you is inherited too, and its age argues the wrong way.** The rule above covers numbers; this one covers written doctrine — a bolded warning, a named failure mode, a documented incident. Those read as more settled than a fresh measurement precisely because someone paid for one once, so they get quoted into briefs and deploy plans without a check. But a claim about behavior describes the code at the moment it was written, and the fix that incident prompted is the very thing most likely to have retired it, silently, because nothing re-reads a paragraph when the behavior beneath it moves. Re-measure before you spend it. When the real thing cannot safely be run, test the layer beneath it and say which half you actually checked rather than which half you believe.

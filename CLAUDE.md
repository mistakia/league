...
**A timestamp populated on only ONE row of a group makes `Number(null)` → `0` read as "infinitely overdue" rather than as an error.** `restricted_free_agency_bids.announced` is written by `announce-restricted-free-agent.mjs` onto the nominating team's own bid and only that bid — every competing bid on the same player carries `announced = null` for its whole life, because the announcement is a property of the PLAYER's window and not of a bid row. `process-restricted-free-agency-bids.mjs` read it per bid, so a competing bid's null coerced to epoch 0, landed roughly 41,000 windows in the past, and came due the instant it was submitted: league 1 processed its second 2026 nomination at 10:10 ET on 2026-08-02 instead of 16:00 ET, three minutes after the winning bid was placed, and cut a player off the winning team's cutlist to make room. Nothing threw, and the script's own shortfall oracle was blind to it in both directions because it filtered `whereNotNull('rfab.announced')` — excluding exactly the rows that were mis-firing. Two rules. When a field is populated on one member of a group, resolve it through the GROUP key (here, the player's nomination) rather than off the row in hand. And refuse a falsy value explicitly — returning `Infinity` — rather than letting it flow into date arithmetic, which is the same silent-`NaN` family as the `current_season` comparator trap above: window math cannot fail loudly on a bad input. Note the suite could not have caught this: `test/scripts.restricted-free-agency.spec.mjs` gave its competing bids an `announced` value they never carry in production, so the fixtures encoded the wrong shape. When a live incident contradicts a green suite, check whether the fixture builds a row the writer never builds.

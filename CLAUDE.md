...
**Put that worktree INSIDE the user-base tree, not in `/tmp` — this repo resolves some dependencies from user-base's ROOT `node_modules`, one level above its own.** `node-fetch` is not installed in `repository/active/league/node_modules` at all; `league-import/espn.mjs` imports it and Node finds it by walking up to `/Users/<you>/user-base/node_modules`. A worktree at `/tmp/<name>` has no such ancestor, so symlinking `node_modules` is not enough and the **full suite dies at load** with `ERR_MODULE_NOT_FOUND: Cannot find package 'node-fetch' imported from .../league-import/espn.mjs` before a single test runs. The failure is easy to misread as your change breaking the suite, and it is louder than it looks in only one direction: the SQL-validity gate and an individual golden spec both run fine from `/tmp`, because neither loads `test/importer-espn.spec.mjs` — so `/tmp` appears to work right up until you ask for the number that matters. Confirmed 2026-07-31; the same commit went from `ERR_MODULE_NOT_FOUND` to 2429 passing purely by relocating the worktree under `~/user-base/tmp/`.

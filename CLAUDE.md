...
**`scripts/update-data-view-snapshots.mjs` takes no filename filter — it rewrites every fixture that currently mismatches.** Two hazards follow. It runs outside `test/global.mjs`, so it has no MockDate and will silently re-bless any clock-derived golden against the real current time. And it will bless unrelated drift from a sibling session's uncommitted edits in a shared tree. Always `git status -- test/data-view-queries/` after regenerating and revert anything you did not intend to touch. `pff-team-grades-next-week-opponent.json` is a live example of a clock-derived golden: it hardcodes a `week =` value that self-breaks on date rollover.

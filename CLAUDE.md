...
**Applying a RENAME or DROP obliges you to commit its consumer sweep in the same sitting — the schema export races you.** `yarn export:schema` mirrors production, not the caller's changes, so between your apply and your commit **any other session** exporting for its own unrelated cluster will dump your DDL into THEIR commit. `db/schema.postgres.sql` on master then declares a column that master's code no longer resolves, and CI goes red under whoever exported next. Neither party can prevent this at the export end: an export that refused to carry foreign DDL would produce a schema file disagreeing with the live database, which is a worse failure than the one it prevents. The window can only be closed at the applying end, by never opening it. **Additive DDL does not open this window** — a new table or column makes the schema a superset of what the code needs, so nothing fails to resolve; that is why a foreign export carrying new tables is harmless while one carrying a rename is not. On 2026-07-29 this fired three times in one afternoon from a single window: a `projections` rename export carried an unswept `nfl_games` retype to master (6 failing tests, and production 500s on two live GET routes once real traffic hit them), while the same mechanism carried two other clusters' new tables harmlessly. Note this is strictly stronger than the deploy rule below — a deploy can lag safely, a commit cannot.

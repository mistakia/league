  ...
  render = () => {
    ...
    const options = {
      ...
      series: [{
        type: 'area',
        name: 'Team',
        data: Object.values(summary.team).map(v => parseFloat(v.toFixed(1))),
        pointPlacement: 'on'
      }, {
        name: 'League Avg.',
        data: Object.values(summary.league_avg).map(v => parseFloat(v.toFixed(1))),
        pointPlacement: 'on'
      }, {
        name: 'Division Avg.',
        data: Object.values(summary.div_avg).map(v => parseFloat(v.toFixed(1))),
        pointPlacement: 'on',
        visible: false
      }],

  ...
  switch (type) {
    ...
    case auctionActions.AUCTION_PROCESSED: {
      ...
      return state.updateIn(
        [payload.tid, constants.week, 'players'],
        (players) =>
          players.push({
            rid,
    ...
    case rosterActions.ROSTER_TRANSACTIONS: {
      return state.withMutations((state) => {
        payload.data.forEach((p) => {
          ...
          const players = state.getIn([t.tid, constants.week, 'players'])
          if (!players) return state

          const key = players.findKey((p) => p.pid === t.pid)
          if (t.type === constants.transactions.ROSTER_RELEASE) {
            return state.deleteIn([t.tid, constants.week, 'players', key])
          } else {
            return state.updateIn([t.tid, constants.week, 'players'], (arr) =>
              arr.push({
                rid: t.rid,
                slot: p.slot,
                pid: t.pid,
    ...
    case rosterActions.PUT_ROSTER_FULFILLED: {
      return state.withMutations((state) => {
        ...
        const players = state.getIn([tid, constants.week, 'players'])
        if (!players) return state

        payload.data.forEach(({ pid, slot }) => {
          const index = players.findIndex((p) => p.pid === pid)
          state.setIn([tid, constants.week, 'players', index, 'slot'], slot)
        })
      })
    }
    ...
    case rosterActions.POST_RESERVE_FULFILLED: {
      return state.withMutations((state) => {
        ...
        if (payload.data.slot) {
          state.setIn(
            [payload.data.tid, constants.week, 'players', index, 'slot'],
            payload.data.slot
          )
          ...
          if (payload.data.transaction) {
            ...
            state.mergeIn(
              [payload.data.tid, constants.week, 'players', index],
              {
                type,
                value,
                timestamp
          ...
          state.deleteIn([payload.data.tid, constants.week, 'players', index])
        }
      })
    }
      ...
      return state.withMutations((state) => {
        for (const teamId in payload.lineups) {
          ...
          for (const week in lineups) {
            ...
            state.setIn([tid, constants.week, 'lineups', week], lineups[week])
          }
        }
      })
    ...
    case teamActions.POST_TEAMS_FULFILLED:
      return state.setIn(
        [payload.data.team.uid, constants.week],
    ...
    case rosterActions.POST_ROSTERS_FULFILLED: {
      ...
      return state.updateIn(
        [payload.opts.teamId, constants.week, 'players'],
        (arr) =>
          arr.push({
            rid,
            tag: 1,
    ...
    case rosterActions.POST_TAG_FULFILLED:
      return state.withMutations((state) => {
        ...
        state.setIn(
          [payload.opts.teamId, constants.week, 'players', index, 'tag'],
          payload.opts.tag
        )
    ...
    case rosterActions.ADD_TRANSITION_TAG: {
      return state.withMutations((state) => {
        const players = state.getIn([payload.teamId, constants.week, 'players'])
        if (!players) return state

        const index = players.findIndex((p) => p.pid === payload.pid)
        state.setIn(
          [payload.teamId, constants.week, 'players', index, 'tag'],
          constants.tags.TRANSITION
        )

        state.setIn(
          [payload.teamId, constants.week, 'players', index, 'bid'],
          payload.bid
        )
    ...
    case rosterActions.REMOVE_TAG: {
      ...
      const index = players.findIndex((p) => p.pid === payload.remove)
      return state.mergeIn([payload.teamId, constants.week, 'players', index], {

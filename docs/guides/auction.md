...
**The write verbs carry the SAME narrowing, and this section said the opposite until 2026-09-05.** It recorded an operator ruling of 2026-09-02 that a commissioner writing another team's election was allowable, and instructed the reader not to narrow the write verbs to match the read. `a53c7a1a0` narrowed them anyway, on a security finding proven by execution: with the narrowing removed, the commissioner's POST for another team returns 200 and writes that team's ceiling, which binds it through `build_auction_claims` to a number it never stated and discharges it from the outstanding set. DELETE is sharper, because withdraw settles in the same transaction — withdrawing a rival's ceiling can hand the player to the commissioner's own team below it, and withdrawing a rival's decline stalls the player with no clock to recover.

So the asymmetry is gone and all three election routes now require OWNERSHIP. The earlier ruling is superseded rather than forgotten, and it is written down here because a guide that still said "do not narrow the write verbs" would talk a future session into reopening the hole.

**One consequence reaches the end-to-end driver.** `scripts/drive-auction-end-to-end.mjs` authenticates every REST call as the commissioner, which worked only because the routes accepted the commissioner for every team. Since the narrowing it can drive elections for exactly the teams that have an owner — on the league 119 mirror that is two of ten — so its elections and settlement scenarios abort on `invalid teamId`. Driving a full field again needs the setup elections seeded directly rather than posted, with the route assertions left on the teams that do have users.

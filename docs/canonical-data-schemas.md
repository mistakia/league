...
| `projections_history` | `season_year::text \|\| '_' \|\| season_type_to_text(season_type) \|\| '_WEEK_' \|\| week::text` | Dated observation history, WEEKLY grain only; `CHECK (week >= 1)` and `week NOT NULL`, so no `_WEEK_0` identifier can be generated; `season_type` is ENUM, converted via `season_type_to_text()`; partitioned by `season_year` |
| `projections_index`   | Same as `projections_history`                                                                    | Current state, WEEKLY grain only; `CHECK (week >= 1)`, so no `_WEEK_0` identifier can be generated; partitioned by `season_year`; requires partition pruning hint                                                              |
| `practice`            | `season_year::text \|\| '_' \|\| season_type \|\| '_WEEK_' \|\| week::text`                      | `season_type` column added with `DEFAULT 'REG'`                                                                                                                                                                                |

**Neither `season_projections_history` nor `season_projections_index` has an `nfl_week_id`, and that is the point of both tables.** The season-long series used to live under a `week = 0` sentinel — the raw history in `projections_history`, generating 2.43 million `YYYY_REG_WEEK_0` identifiers, and the current state in `projections_index`, generating ~50,000 more — a spelling the season vocabulary does not admit, minted only because a mixed table demanded a week. The history series is keyed `(source_id, pid, season_year, generated_at)`; the current-state table is keyed `(source_id, pid, season_year)`. Neither carries `week`, `season_type`, `nfl_week_id` or `user_id`. Do not add one: a season-long projection has no week to identify.

**A week predicate is what made this dangerous, not the identifier.** With the season row stored at `week = 0` on a table whose every other row is a real fantasy week, any week floor could amputate it — and on 2026-08-04 one did, silently, pricing `market_salary` at $0 on 22 of 23 league formats. Splitting the tables makes that failure unavailable rather than guarded: there is no column for a predicate to reference.

...
**A fact keyed by (subject, year, week) resolves its week scope through `week-scoped-cte.mjs`, never inline.** Three decisions have to agree — how many weeks the CTE spans, whether the JOIN correlates on week, and what the table alias hashes — and each way they can drift is silent: a pinned CTE under a week axis broadcasts one week's value onto every week row, an uncorrelated join fans the cell into one row per week, and an alias coarser than the CTE collapses two columns onto one join group. `resolve_week_scope`, `week_scope_alias_key` and `correlate_week_scoped_cte` are one resolution shared by all three. Note `week_split` is derived from `week_reference`, not from `row_axes`: a column at the base `player` grain is handed an EMPTY `row_axes` even under a week-axis request.
...
- `sort_expressions({params, column_index, table_name, data_view_options})` — the SQL expressions to ORDER BY, for a column the select-ordinal sort lookup cannot resolve. That lookup can only find a projection carrying a `<name>_<column_index>` alias, and when it finds nothing it **drops the sort in silence** rather than failing. `player_name` (two bare identifiers) and `player_league_roster_status` (an alias with no index suffix) were both unsortable that way, and pid order looks enough like alphabetical order to hide it.

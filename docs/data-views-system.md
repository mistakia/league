...
- **Confirm** — `scripts/contribution-reproduce.mjs` re-executes a report's captured `table_state` against production through this envelope. Production is the microscope: it is the only substrate that reliably contains the offending row. Its outcome vocabulary keeps `allowlist_gap` distinct from `no_rows`, because conflating them closes a real bug as user error.

...
**A malformed value throws; it does not degrade.** Validation is three steps — an `MM-DD` shape regex, then `sql_integer_param` on each half, then a month 1-12 / day 1-31 range check — and all three are required, since `sql_integer_param` carries no range check and `13-01` would otherwise reach `make_date` and raise at execution. The throw is raised through `invalid_data_view_request`, which sets `is_invalid_request` — the four data-view routes map that to 400, and over the websocket (the SPA's actual transport) it rides a `DATA_VIEW_ERROR` frame carrying the flag, which is what lets the page SHOW the message instead of the generic banner it shows for an unclassified error. This matches the shipped behaviour for a malformed `year` on the same column.
...
### The reproduction substrate, and the correctness oracle

The second consumer runs in two steps, and the split exists because a test that reads production cannot live in continuous integration.

- **Confirm** — `scripts/contribution-reproduce.mjs` re-executes a report's captured `table_state` against production through this envelope. Production is the microscope: it is the only substrate that reliably contains the offending row. Two distinctions in its outcome vocabulary exist because collapsing either produces a confident wrong answer: `allowlist_gap` is not `no_rows`, because conflating them closes a real bug as user error; and `stale_request` is neither, because the builder silently drops an unrecognized `column_id` and would otherwise answer a question the submitter never asked. The stale check runs against the registry before generation, since afterwards the dropped column leaves no trace.
- **Distill** — `scripts/contribution-distill.mjs` extracts the minimal rows that query depended on into a `result_equivalence` fixture under `test/data-view-queries/`. The fixture is the artifact; production is never in the loop again.

**The regression harness is a DIFFERENTIAL oracle and a bug report needs a correctness one.** `scripts/data-view-regression-check.mjs` answers "did my change alter the results" by hashing rows across two revisions. A report asserts "the current result is WRONG", which a differential cannot express — it is silent when both revisions are wrong together, which is exactly the case a report describes.

A report's claim becomes an assertion in one of two admissible forms, and they are the two `result_equivalence` already implements:

- `reference_sql` — an independent derivation, recomputing the reported quantity in SQL that shares no code path with the query builder. **An invariant is this form, not a third one**: "a rate lies in [0,1]", "the sum over partitions equals the sum over the whole", "a `year_offset` column equals that column queried at the offset year directly" are each a SELECT returning what the correct answer must be. That folding is why reproduction needs no extension to the harness.
- `expected_rows` — a hand-derived value, admissible only against a seed small enough for a reviewer to check by inspection. Distillation authors the seed, so this is usually satisfiable.

**The prohibited form is `expected_rows` populated with the output of the code under test.** That is a characterization test: green on master by construction, never the red half of a red-then-green gate, and it pins the defect as correct behaviour permanently. It is also the easiest fixture to produce, because the confirm step just handed you those rows — so the prohibition is mechanical rather than advisory. A distilled fixture is admissible only if it is RED on the revision the bug was reported against, which excludes a characterization oracle without trusting any provenance field an author could fill in wrongly.

Distillation therefore runs in two phases. Phase 1 emits the fixture with a characterization oracle and requires it GREEN — that measures the SEED, proving the offline query saw everything the production query saw, and its fixture carries a `phase: seed-check` marker that makes it uncommittable. Phase 2 substitutes a real oracle and requires it RED. A phase-1 red means the seed is incomplete and nothing has been proven; a phase-2 green means either the oracle is still a characterization or the defect is already fixed, and the confirm step's observation is what distinguishes them.

Full reasoning lives in the header of `scripts/contribution-distill.mjs`, next to the code that enforces it.

...
The `points` columns (`player_{week,season,rest_of_season}_projected_points`) also accept the `sourceid` param: they are now **computed in-query** from the same `projections_index`/`ros_projections` rows (scored by the selected scoring format's weights) rather than joined from the precomputed `scoring_format_player_projection_points` table. This makes projected points honor the source picker and stay self-consistent with the raw-stat columns shown beside them. The scorer is inlined in `libs-server/data-views-column-definitions/player-projected-column-definitions.mjs` (`projection_fantasy_points_sql`) and faithfully implements `calculatePoints({ use_projected_stats: true })`: `projected_base_stats` × format weights, a position CASE for non-uniform reception formats, `+ xpm`, the field-goal distance buckets (`3·fg19 + 3·fg29 + 3·fg39 + 4·fg49 + 5·fg50`, never `fgy/10`), and the full DST block. **The displayed point values are NOT value-identical to the legacy precomputed column** — the precomputed total and `projections_index` are independently-timed snapshots of the average projection. For a frozen preseason snapshot they match (verified: 2025 season week-0, AVERAGE/draftkings, the in-query scorer reproduces the stored total **989/989, max diff 0.00**). They diverge where the snapshots were taken at different times or where `projections_index` no longer carries the row: notably **historical weekly** rows, because `projections_index` is forward-looking and does not retain a past week's projection (its weekly row zeroes out) while the precomputed total froze that week's value. Example: Ja'Marr Chase, 2025 week 12 — `projections_index` AVERAGE is all-zero so in-query points are `0.00`, vs a frozen precomputed `18.75`. This is intentional: the in-query value now agrees with the projected raw-stat columns beside it (his projected `rec`/`rec_yds`/`rec_tds` also render 0 for that week), which the legacy `18.75` contradicted. For live/current weeks both tables are written from the same run and agree. Post-deploy non-degeneracy check (run against production): for a season's actively-projected players (any non-zero base stat), in-query AVERAGE points should be `> 0` and within a plausible range for ≥95% — measured 623/623 (100%) for 2024.

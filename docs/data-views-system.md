...
**Defensive-unit reasoning**: `rate-type-per-team-play.mjs` uses a single join expression regardless of `team_unit` ('off' or 'def'). `player_gamelogs.nfl_team` stores the player's own team; for defensive players that equals the defensive team on `nfl_plays.defense_nfl_team`, so no branch is needed.

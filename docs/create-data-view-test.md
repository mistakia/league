...
Two limits worth knowing before you trust a clean run. It cannot measure the 31 fixtures that reach the database during query generation, and it names them rather than folding them into the clean total. And **run it from a worktree pinned to the pushed ref when the shared tree is dirty** — the seventh fixture above was invisible from the shared checkout, where a sibling's uncommitted edit to a column definition made it read as an ordinary red instead of as drift.

...
**Run it with a database, or it under-reports.** Without one, 31 fixtures cannot be generated at all; the script names them rather than folding them into the clean total. That is not a theoretical gap — re-running the first sweep with a database found **two more** drifted fixtures inside it, both moving the same hash, taking the total from 7 to 9. All nine are blessed and the corpus now reads 287 of 287 byte-identical. Use `yarn test:isolated`'s own database rather than hand-assembling a `LEAGUE_DB_*` invocation, which is the trap CLAUDE.md names: pointing at a database that does not exist hangs indefinitely and prints nothing.

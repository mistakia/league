...
**To sweep the WHOLE corpus for hashes that already moved, run `node scripts/audit-data-view-alias-drift.mjs`** (`NODE_ENV=test TZ=America/New_York`, and it takes `--json`). It regenerates every fixture at the golden clock and reports the ones whose stored alias the tree will not emit again — the drift the two comparison paths above are blind to by construction. It is deliberately NOT wired into `yarn check:cluster` or CI: a red master defers every session's push to this repo fleet-wide, and an alias moves legitimately during any column migration, so a blocking gate here would park unrelated work behind routine churn. Run it when you touch an alias input; re-bless what it names.

The first sweep (2026-09-01) found 7 of 256 generable fixtures storing an unreproducible alias, and the split is the useful part. Sweeping six of them against 157 candidate blessing clocks showed 2 were ordinary pre-pin residue — reproducible at any clock before the corpus was pinned — while **4 were reproducible at no clock at all**, meaning the alias input had changed in code and nothing could report it. So "the fixture was blessed at the wrong time of year" explains a minority of it; the clock half is already solved by the unconditional pin in `test/utils/golden-clock.mjs`, and the code half is what the script is for.

Two limits worth knowing before you trust a clean run, both of which bit on the first sweep.

**Run it with a database when you want the whole corpus.** Without one, 31 fixtures cannot be generated at all; the script names them rather than folding them into the clean total. Running with a database found **two more drifted fixtures** the clean run could not see — `test-condition-for-scoring-format-id-param-multiple-fantasy-points-from-play.json` and `year-split-with-multiple-rate-type-with-statements.json`, both moving the same hash. Those two are still **unblessed**. Use `yarn test:isolated`'s own database rather than hand-assembling a `LEAGUE_DB_*` invocation, which is the trap CLAUDE.md names: pointing at a database that does not exist hangs indefinitely and prints nothing.

**Run it from a worktree pinned to the pushed ref when the shared tree is dirty.** The seventh fixture above was invisible from the shared checkout, where a sibling's uncommitted edit to a column definition made it read as an ordinary red instead of as drift — the dirt decides what you find.

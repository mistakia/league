...
describe('player_nfl_teams routing', function () {
  ...
  it('routes to CTE when nfl_week_id is set (multi-year or week-specific)', () => {
    ...
    expect(def.is_where_column_array({ params, splits: [] })).to.equal(true)
    const select = def.main_select({ params, splits: [], column_index: 0 })
  ...
  it('routes to CTE when splits are set', () => {
    expect(
      def.is_where_column_array({ params: {}, splits: ['year'] })
    ).to.equal(true)
  })

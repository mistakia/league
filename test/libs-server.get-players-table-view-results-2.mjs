...
describe('LIBS SERVER get_data_view_results', () => {
  ...
  it('team field with matchup_opponent_type param', () => {
    ...
    const expected_query = `with "current_week_opponents" as (select "h" as "nfl_team", "v" as "opponent" from "public"."nfl_games" where "year" = ${constants.season.stats_season_year} and "week" = ${constants.season.week} and "seas_type" = 'REG' union select "v" as "nfl_team", "h" as "opponent" from "public"."nfl_games" where "year" = ${constants.season.stats_season_year} and "week" = ${constants.season.week} and "seas_type" = 'REG'), "t4f71f555ce1e3fa469a3fc595eca1a52" as (select "player_gamelogs"."pid", count(*) as "rate_type_total_count", array_agg(distinct player_gamelogs.tm) as teams from "player_gamelogs" left join "nfl_games" on "nfl_games"."esbid" = "player_gamelogs"."esbid" where "nfl_games"."seas_type" = 'REG' and "player_gamelogs"."active" = true and "nfl_games"."year" in (${constants.season.year}) group by "player_gamelogs"."pid"), "te9ef1f7b73db99cae1b6e1e7b8c1063f" as (select "year", "week", v as nfl_team, h as game_opponent, true as game_is_home from "nfl_games" where "year" in (${constants.season.year}) and "week" in (${constants.season.week}) and "seas_type" = 'REG' and h IN ('WAS', 'ARI', 'LAR') union select "year", "week", h as nfl_team, v as game_opponent, false as game_is_home from "nfl_games" where "year" in (${constants.season.year}) and "week" in (${constants.season.week}) and "seas_type" = 'REG' and v IN ('WAS', 'ARI', 'LAR')), "taa5c3680ab3ee27a83360c4448200e28" as (select "nfl_plays"."def" as "nfl_team", "nfl_plays"."year", "nfl_plays"."week", SUM(pass_yds) AS team_pass_yds_from_plays from "nfl_plays" where not "play_type" = 'NOPL' and "nfl_plays"."seas_type" = 'REG' and "nfl_plays"."year" in (${constants.season.year}) group by "nfl_plays"."def", "nfl_plays"."year", "nfl_plays"."week"), "taa5c3680ab3ee27a83360c4448200e28_team_stats" as (select "taa5c3680ab3ee27a83360c4448200e28"."nfl_team", sum(taa5c3680ab3ee27a83360c4448200e28.team_pass_yds_from_plays) as team_pass_yds_from_plays from "taa5c3680ab3ee27a83360c4448200e28" where "taa5c3680ab3ee27a83360c4448200e28"."year" in (2024) group by "taa5c3680ab3ee27a83360c4448200e28"."nfl_team") select "player"."pid", player.fname, player.lname, "player"."pos" AS "pos_0", CAST(taa5c3680ab3ee27a83360c4448200e28_team_stats.team_pass_yds_from_plays AS DECIMAL) / NULLIF(CAST(t4f71f555ce1e3fa469a3fc595eca1a52.rate_type_total_count AS DECIMAL), 0) AS "team_pass_yds_from_plays_0", "player"."pos" from "player" inner join "current_week_opponents" on "player"."current_nfl_team" = "current_week_opponents"."nfl_team" left join "t4f71f555ce1e3fa469a3fc595eca1a52" on "t4f71f555ce1e3fa469a3fc595eca1a52"."pid" = "player"."pid" inner join "te9ef1f7b73db99cae1b6e1e7b8c1063f" on "te9ef1f7b73db99cae1b6e1e7b8c1063f"."nfl_team" = "player"."current_nfl_team" left join "taa5c3680ab3ee27a83360c4448200e28_team_stats" on "taa5c3680ab3ee27a83360c4448200e28_team_stats"."nfl_team" = "current_week_opponents"."opponent" group by player.fname, player.lname, "player"."pos", "taa5c3680ab3ee27a83360c4448200e28_team_stats"."team_pass_yds_from_plays", t4f71f555ce1e3fa469a3fc595eca1a52.rate_type_total_count, "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 5 DESC NULLS LAST, "player"."pid" asc limit 500`
    compare_queries(query.toString(), expected_query)
  })

...
describe('build_active_restricted_free_agency_bids_query', function () {
  ...
  it('does not constrain ownership -- the call site decides', () => {
    ...
    // the team's own players and joins the nomination itself.
    expect(sql).to.not.include('original_team_id')
  })
  ...
  it('composes with an ownership filter without losing the liveness guards', () => {
    const own_player_sql = build_active_restricted_free_agency_bids_query({
      ...
      .where('restricted_free_agency_nominations.original_team_id', 12)
      .toString()

    expect(own_player_sql).to.include('"original_team_id" = 12')
  ...
  describe('call sites', function () {
    ...
    //
    // Note both roster loaders legitimately keep a SECOND query, against
    // `restricted_free_agency_nominations`, reporting the state of a tag. It
    // reads the window timestamps rather than pricing anything, so "no other
    // query in this file" would be the wrong assertion.

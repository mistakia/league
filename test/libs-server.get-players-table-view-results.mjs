...
describe('LIBS SERVER get_players_table_view_results', () => {
  ...
  it('should handle player_target_share_from_plays', () => {
    ...
    const expected_query = `with "t7f8e2aa2c1c4d6e00dacfa7a46010596" as (select "pg"."pid", ROUND(100.0 * COUNT(CASE WHEN nfl_plays.trg_pid = pg.pid THEN 1 ELSE NULL END) / COUNT(*), 2) as trg_share_from_plays from "nfl_plays" inner join "player_gamelogs" as "pg" on "nfl_plays"."esbid" = "pg"."esbid" and "nfl_plays"."off" = "pg"."tm" where not "play_type" = 'NOPL' and "seas_type" = 'REG' and "trg_pid" is not null and "nfl_plays"."year" in (2023) group by "pg"."pid" having ROUND(100.0 * COUNT(CASE WHEN nfl_plays.trg_pid = pg.pid THEN 1 ELSE NULL END) / COUNT(*), 2) >= '25') select "player"."pid", "t7f8e2aa2c1c4d6e00dacfa7a46010596"."trg_share_from_plays" AS "trg_share_from_plays_0", player.fname, player.lname, "player"."pos" from "player" inner join "t7f8e2aa2c1c4d6e00dacfa7a46010596" on "t7f8e2aa2c1c4d6e00dacfa7a46010596"."pid" = "player"."pid" group by "t7f8e2aa2c1c4d6e00dacfa7a46010596"."trg_share_from_plays", player.fname, player.lname, "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 2 DESC NULLS LAST limit 500`
    expect(query.toString()).to.equal(expected_query)
  })
  ...
  it('should handle player_target_share_from_plays - different params', () => {
    ...
    const expected_query = `with "t7f8e2aa2c1c4d6e00dacfa7a46010596" as (select "pg"."pid", ROUND(100.0 * COUNT(CASE WHEN nfl_plays.trg_pid = pg.pid THEN 1 ELSE NULL END) / COUNT(*), 2) as trg_share_from_plays from "nfl_plays" inner join "player_gamelogs" as "pg" on "nfl_plays"."esbid" = "pg"."esbid" and "nfl_plays"."off" = "pg"."tm" where not "play_type" = 'NOPL' and "seas_type" = 'REG' and "trg_pid" is not null and "nfl_plays"."year" in (2023) group by "pg"."pid" having ROUND(100.0 * COUNT(CASE WHEN nfl_plays.trg_pid = pg.pid THEN 1 ELSE NULL END) / COUNT(*), 2) >= '25'), "tf8f3a32ec3dc1413437cdf8a84874d57" as (select "pg"."pid", ROUND(100.0 * COUNT(CASE WHEN nfl_plays.trg_pid = pg.pid THEN 1 ELSE NULL END) / COUNT(*), 2) as trg_share_from_plays from "nfl_plays" inner join "player_gamelogs" as "pg" on "nfl_plays"."esbid" = "pg"."esbid" and "nfl_plays"."off" = "pg"."tm" where not "play_type" = 'NOPL' and "seas_type" = 'REG' and "trg_pid" is not null and "nfl_plays"."week" in (1, 2, 3) and "nfl_plays"."year" in (2022) group by "pg"."pid") select "player"."pid", "t7f8e2aa2c1c4d6e00dacfa7a46010596"."trg_share_from_plays" AS "trg_share_from_plays_0", player.fname, player.lname, "tf8f3a32ec3dc1413437cdf8a84874d57"."trg_share_from_plays" AS "trg_share_from_plays_1", "player"."pos" from "player" inner join "t7f8e2aa2c1c4d6e00dacfa7a46010596" on "t7f8e2aa2c1c4d6e00dacfa7a46010596"."pid" = "player"."pid" left join "tf8f3a32ec3dc1413437cdf8a84874d57" on "tf8f3a32ec3dc1413437cdf8a84874d57"."pid" = "player"."pid" group by "t7f8e2aa2c1c4d6e00dacfa7a46010596"."trg_share_from_plays", player.fname, player.lname, "tf8f3a32ec3dc1413437cdf8a84874d57"."trg_share_from_plays", "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 2 DESC NULLS LAST limit 500`
    expect(query.toString()).to.equal(expected_query)
  })
  ...
  it('should create a split query — year', () => {
    ...
    const expected_query = `with "t1ceac1f694bf82585921df0bec8fcc28" as (select "bc_pid", "nfl_plays"."year", SUM(rush_yds) as rush_yds_from_plays_0 from "nfl_plays" where not "play_type" = 'NOPL' and "seas_type" = 'REG' and "nfl_plays"."year" in (2020, 2021, 2022, 2023) group by "bc_pid", "nfl_plays"."year") select "player"."pid", player.fname, player.lname, "t1ceac1f694bf82585921df0bec8fcc28"."rush_yds_from_plays_0" as "rush_yds_from_plays_0", COALESCE(t1ceac1f694bf82585921df0bec8fcc28.year) AS year, "player"."pos" from "player" left join "t1ceac1f694bf82585921df0bec8fcc28" on "t1ceac1f694bf82585921df0bec8fcc28"."bc_pid" = "player"."pid" group by player.fname, player.lname, "t1ceac1f694bf82585921df0bec8fcc28"."rush_yds_from_plays_0", COALESCE(t1ceac1f694bf82585921df0bec8fcc28.year), "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 4 DESC NULLS LAST limit 500`
    expect(query.toString()).to.equal(expected_query)
  })
  ...
  it('should create a query with fields from league format careerlogs with a where filter', () => {
    const query = get_players_table_view_results({
      ...
      where: [
        {
          column_id: 'player_points_added_first_season_from_careerlogs',
          operator: '>=',
          value: '50'
        }
      ],
    ...
    const expected_query = `select "player"."pid", "t2c88ab25d4acbc66daf6137b64987326"."points_added_first_seas" AS "points_added_first_seas_from_careerlogs_0", player.fname, player.lname, "player"."pos" AS "pos_0", CASE
    WHEN rosters_players.slot = 13 THEN 'injured_reserve'
    WHEN rosters_players.slot = 12 THEN 'practice_squad'
    WHEN rosters_players.slot IS NULL THEN 'free_agent'
    ELSE 'active_roster'
  END AS player_league_roster_status, rosters_players.slot, rosters_players.tid, rosters_players.tag, "t0984699909800a4c1372fbe19abf07af"."top_3" AS "top_3_from_careerlogs_0", "t0984699909800a4c1372fbe19abf07af"."points_per_game" AS "points_per_game_from_careerlogs_0", "t0984699909800a4c1372fbe19abf07af"."top_6" AS "top_6_from_careerlogs_0", "t0984699909800a4c1372fbe19abf07af"."top_12" AS "top_12_from_careerlogs_0", "t0984699909800a4c1372fbe19abf07af"."top_24" AS "top_24_from_careerlogs_0", "t0984699909800a4c1372fbe19abf07af"."top_36" AS "top_36_from_careerlogs_0", "player"."pos" from "player" inner join "league_format_player_careerlogs" as "t2c88ab25d4acbc66daf6137b64987326" on "t2c88ab25d4acbc66daf6137b64987326"."pid" = "player"."pid" and t2c88ab25d4acbc66daf6137b64987326.league_format_hash = '1985e1968b75707ebcab9da620176a0b218c5c1bd28d00cbbc4d1744a1631d0b' left join "rosters_players" on "rosters_players"."pid" = "player"."pid" and "rosters_players"."year" = 2024 and "rosters_players"."week" = 0 and "rosters_players"."lid" = 1 left join "scoring_format_player_careerlogs" as "t0984699909800a4c1372fbe19abf07af" on "t0984699909800a4c1372fbe19abf07af"."pid" = "player"."pid" and t0984699909800a4c1372fbe19abf07af.scoring_format_hash = '0df3e49bb29d3dbbeb7e9479b9e77f2688c0521df4e147cd9035f042680ba13d' where points_added_first_seas >= '50' group by "t2c88ab25d4acbc66daf6137b64987326"."points_added_first_seas", player.fname, player.lname, "player"."pos", rosters_players.slot, rosters_players.tid, rosters_players.tag, "t0984699909800a4c1372fbe19abf07af"."top_3", "t0984699909800a4c1372fbe19abf07af"."points_per_game", "t0984699909800a4c1372fbe19abf07af"."top_6", "t0984699909800a4c1372fbe19abf07af"."top_12", "t0984699909800a4c1372fbe19abf07af"."top_24", "t0984699909800a4c1372fbe19abf07af"."top_36", "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 10 DESC NULLS LAST limit 500`
    expect(query.toString()).to.equal(expected_query)
  })

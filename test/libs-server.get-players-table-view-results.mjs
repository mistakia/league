...
describe('LIBS SERVER get_players_table_view_results', () => {
  ...
  it('should generate a query with a team stat column', () => {
    ...
    const expected_query = `with "t7ce618d9efd1bad910446e01527397b9" as (select "pg"."pid", ROUND(100.0 * COUNT(CASE WHEN nfl_plays.trg_pid = pg.pid THEN 1 ELSE NULL END) / NULLIF(SUM(CASE WHEN trg_pid IS NOT NULL THEN 1 ELSE 0 END), 0), 2) as trg_share_from_plays from "nfl_plays" inner join "player_gamelogs" as "pg" on "nfl_plays"."esbid" = "pg"."esbid" and "nfl_plays"."off" = "pg"."tm" where not "play_type" = 'NOPL' and "nfl_plays"."seas_type" = 'REG' and ("trg_pid" is not null) and "nfl_plays"."year" in (2023) group by "pg"."pid"), "td157a7e10f8d9d979b7fd01562896653" as (select "nfl_plays"."off" as "nfl_team", "nfl_plays"."year", "nfl_plays"."week", SUM(pass_yds) AS team_pass_yds_from_plays_0 from "nfl_plays" where not "play_type" = 'NOPL' and "nfl_plays"."seas_type" = 'REG' and "nfl_plays"."year" in (2023) group by "nfl_plays"."off", "nfl_plays"."year", "nfl_plays"."week"), "td157a7e10f8d9d979b7fd01562896653_player_team_stats" as (select "player_gamelogs"."pid", sum(td157a7e10f8d9d979b7fd01562896653.team_pass_yds_from_plays_0) as team_pass_yds_from_plays_0 from "player_gamelogs" inner join "nfl_games" on "player_gamelogs"."esbid" = "nfl_games"."esbid" inner join "td157a7e10f8d9d979b7fd01562896653" on "player_gamelogs"."tm" = "td157a7e10f8d9d979b7fd01562896653"."nfl_team" and "nfl_games"."year" = "td157a7e10f8d9d979b7fd01562896653"."year" and "nfl_games"."week" = "td157a7e10f8d9d979b7fd01562896653"."week" where "nfl_games"."year" in (2023) group by "player_gamelogs"."pid") select "player"."pid", player.fname, player.lname, "t7ce618d9efd1bad910446e01527397b9"."trg_share_from_plays" AS "trg_share_from_plays_0", "td157a7e10f8d9d979b7fd01562896653_player_team_stats"."team_pass_yds_from_plays_0" as "team_pass_yds_from_plays_0", "player"."pos" from "player" left join "t7ce618d9efd1bad910446e01527397b9" on "t7ce618d9efd1bad910446e01527397b9"."pid" = "player"."pid" left join "td157a7e10f8d9d979b7fd01562896653_player_team_stats" on "player"."pid" = "td157a7e10f8d9d979b7fd01562896653_player_team_stats"."pid" group by player.fname, player.lname, "t7ce618d9efd1bad910446e01527397b9"."trg_share_from_plays", "td157a7e10f8d9d979b7fd01562896653_player_team_stats"."team_pass_yds_from_plays_0", "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 4 DESC NULLS LAST, "player"."pid" asc limit 500`
    expect(query.toString()).to.equal(expected_query)
  })
  ...
  it('should create a keeptradecut query with splits', () => {
    ...
    const expected_query = `select "player"."pid", player.fname, player.lname, "tc78572ffc09a69493a48490142c672da"."v" AS "player_keeptradecut_value_0", COALESCE(EXTRACT(YEAR FROM TO_TIMESTAMP(tc78572ffc09a69493a48490142c672da.d))) AS year, "player"."pos" from "player" left join "keeptradecut_rankings" as "tc78572ffc09a69493a48490142c672da" on "tc78572ffc09a69493a48490142c672da"."pid" = "player"."pid" and "tc78572ffc09a69493a48490142c672da"."qb" = 2 and "tc78572ffc09a69493a48490142c672da"."type" = 1 and tc78572ffc09a69493a48490142c672da.d IN (EXTRACT(EPOCH FROM to_timestamp('2021-09-01', 'YYYY-MM-DD') AT TIME ZONE 'UTC')::integer, EXTRACT(EPOCH FROM to_timestamp('2022-09-01', 'YYYY-MM-DD') AT TIME ZONE 'UTC')::integer, EXTRACT(EPOCH FROM to_timestamp('2023-09-01', 'YYYY-MM-DD') AT TIME ZONE 'UTC')::integer, EXTRACT(EPOCH FROM to_timestamp('2024-09-01', 'YYYY-MM-DD') AT TIME ZONE 'UTC')::integer) group by player.fname, player.lname, "tc78572ffc09a69493a48490142c672da"."v", COALESCE(EXTRACT(YEAR FROM TO_TIMESTAMP(tc78572ffc09a69493a48490142c672da.d))), "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 4 DESC NULLS LAST, "player"."pid" asc limit 500`
    expect(query.toString()).to.equal(expected_query)
  })

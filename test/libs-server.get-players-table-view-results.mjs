...
describe('LIBS SERVER get_players_table_view_results', () => {
  ...
  it('year splits with sort by age', () => {
    ...
    const expected_query = `with "t4530f8545d18b50db01ac75501e7bf42" as (select "pg"."pid", ROUND((1.5 * COUNT(CASE WHEN nfl_plays.trg_pid = pg.pid THEN 1 ELSE NULL END) / NULLIF(SUM(CASE WHEN trg_pid IS NOT NULL THEN 1 ELSE 0 END), 0)) + (0.7 * SUM(CASE WHEN nfl_plays.trg_pid = pg.pid THEN nfl_plays.dot ELSE 0 END) / NULLIF(SUM(nfl_plays.dot), 0)), 4) as weighted_opp_rating_from_plays, "nfl_plays"."year" from "nfl_plays" inner join "player_gamelogs" as "pg" on "nfl_plays"."esbid" = "pg"."esbid" and "nfl_plays"."off" = "pg"."tm" where not "play_type" = 'NOPL' and "nfl_plays"."seas_type" = 'REG' and ("trg_pid" is not null) and "nfl_plays"."year" in (2017, 2018, 2019, 2020, 2021, 2022, 2023) group by "pg"."pid", "nfl_plays"."year") select "player"."pid", player.fname, player.lname, "player"."ngs_draft_grade" AS "ngs_draft_grade_0", "t4530f8545d18b50db01ac75501e7bf42"."weighted_opp_rating_from_plays" AS "weighted_opp_rating_from_plays_0", "player_seasonlogs"."espn_overall_score" AS "espn_overall_score_0", CASE WHEN player.dob IS NULL OR player.dob = '0000-00-00' THEN NULL ELSE ROUND(EXTRACT(YEAR FROM AGE(opening_days.opening_day, TO_DATE(player.dob, 'YYYY-MM-DD'))) + (EXTRACT(MONTH FROM AGE(opening_days.opening_day, TO_DATE(player.dob, 'YYYY-MM-DD'))) / 12.0) + (EXTRACT(DAY FROM AGE(opening_days.opening_day, TO_DATE(player.dob, 'YYYY-MM-DD'))) / 365.25), 2) END as age_0, COALESCE(t4530f8545d18b50db01ac75501e7bf42.year, player_seasonlogs.year) AS year, "player"."pos" from "player" left join "t4530f8545d18b50db01ac75501e7bf42" on "t4530f8545d18b50db01ac75501e7bf42"."pid" = "player"."pid" and t4530f8545d18b50db01ac75501e7bf42.year IN (2017,2018,2019,2020,2021,2022,2023) left join "player_seasonlogs" on "player_seasonlogs"."pid" = "player"."pid" and player_seasonlogs.year = t4530f8545d18b50db01ac75501e7bf42.year and "player_seasonlogs"."seas_type" = 'REG' left join "opening_days" on "opening_days"."year" = "t4530f8545d18b50db01ac75501e7bf42"."year" where player.pos IN ('WR') group by player.fname, player.lname, "player"."ngs_draft_grade", "t4530f8545d18b50db01ac75501e7bf42"."weighted_opp_rating_from_plays", "player_seasonlogs"."espn_overall_score", player.dob, opening_days.opening_day, COALESCE(t4530f8545d18b50db01ac75501e7bf42.year, player_seasonlogs.year), "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 7 DESC NULLS LAST, "player"."pid" asc limit 500`
    expect(query.toString()).to.equal(expected_query)
  })
  ...
  it('should query WR fantasy stats with year splits and sort by points per game, seasonlogs, careerlogs, plays, and keeptradecut', () => {
    ...
    const expected_query = `with "td39c8430c6b71a1d059ec34d508f2710" as (select "pg"."pid", ROUND((1.5 * COUNT(CASE WHEN nfl_plays.trg_pid = pg.pid THEN 1 ELSE NULL END) / NULLIF(SUM(CASE WHEN trg_pid IS NOT NULL THEN 1 ELSE 0 END), 0)) + (0.7 * SUM(CASE WHEN nfl_plays.trg_pid = pg.pid THEN nfl_plays.dot ELSE 0 END) / NULLIF(SUM(nfl_plays.dot), 0)), 4) as weighted_opp_rating_from_plays, "nfl_plays"."year" from "nfl_plays" inner join "player_gamelogs" as "pg" on "nfl_plays"."esbid" = "pg"."esbid" and "nfl_plays"."off" = "pg"."tm" where not "play_type" = 'NOPL' and "nfl_plays"."seas_type" = 'REG' and ("trg_pid" is not null) and "nfl_plays"."year" in (2018, 2019, 2020, 2021, 2022, 2023) group by "pg"."pid", "nfl_plays"."year") select "player"."pid", player.fname, player.lname, "t0984699909800a4c1372fbe19abf07af"."games" AS "games_from_careerlogs_0", "t8a4a84e735183537b2ba13726efd3e32"."points_per_game" AS "points_per_game_from_seasonlogs_0", "td39c8430c6b71a1d059ec34d508f2710"."weighted_opp_rating_from_plays" AS "weighted_opp_rating_from_plays_0", "player_seasonlogs"."espn_overall_score" AS "espn_overall_score_0", "t95d728bf530769e3ed5859fbedf01162"."v" AS "player_keeptradecut_value_0", COALESCE(t8a4a84e735183537b2ba13726efd3e32.year, td39c8430c6b71a1d059ec34d508f2710.year, player_seasonlogs.year, EXTRACT(YEAR FROM TO_TIMESTAMP(t95d728bf530769e3ed5859fbedf01162.d))) AS year, "player"."pos" from "player" left join "scoring_format_player_careerlogs" as "t0984699909800a4c1372fbe19abf07af" on "t0984699909800a4c1372fbe19abf07af"."pid" = "player"."pid" and t0984699909800a4c1372fbe19abf07af.scoring_format_hash = '0df3e49bb29d3dbbeb7e9479b9e77f2688c0521df4e147cd9035f042680ba13d' left join "scoring_format_player_seasonlogs" as "t8a4a84e735183537b2ba13726efd3e32" on "t8a4a84e735183537b2ba13726efd3e32"."pid" = "player"."pid" and t8a4a84e735183537b2ba13726efd3e32.year IN (2023,2022,2021,2020,2019,2018,2017) and t8a4a84e735183537b2ba13726efd3e32.scoring_format_hash = '0df3e49bb29d3dbbeb7e9479b9e77f2688c0521df4e147cd9035f042680ba13d' left join "td39c8430c6b71a1d059ec34d508f2710" on "td39c8430c6b71a1d059ec34d508f2710"."pid" = "player"."pid" and td39c8430c6b71a1d059ec34d508f2710.year = t8a4a84e735183537b2ba13726efd3e32.year left join "player_seasonlogs" on "player_seasonlogs"."pid" = "player"."pid" and player_seasonlogs.year = t8a4a84e735183537b2ba13726efd3e32.year and "player_seasonlogs"."seas_type" = 'REG' left join "opening_days" on "opening_days"."year" = COALESCE(t8a4a84e735183537b2ba13726efd3e32.year, td39c8430c6b71a1d059ec34d508f2710.year, player_seasonlogs.year) left join "keeptradecut_rankings" as "t95d728bf530769e3ed5859fbedf01162" on "t95d728bf530769e3ed5859fbedf01162"."pid" = "player"."pid" and "t95d728bf530769e3ed5859fbedf01162"."qb" = 2 and "t95d728bf530769e3ed5859fbedf01162"."type" = 1 and "t95d728bf530769e3ed5859fbedf01162"."d" = EXTRACT(EPOCH FROM (date_trunc('day', opening_days.opening_day) + interval '1 year'))::integer and opening_days.year = (t8a4a84e735183537b2ba13726efd3e32.year) where player.pos IN ('WR') group by player.fname, player.lname, "t0984699909800a4c1372fbe19abf07af"."games", "t8a4a84e735183537b2ba13726efd3e32"."points_per_game", "td39c8430c6b71a1d059ec34d508f2710"."weighted_opp_rating_from_plays", "player_seasonlogs"."espn_overall_score", "t95d728bf530769e3ed5859fbedf01162"."v", COALESCE(t8a4a84e735183537b2ba13726efd3e32.year, td39c8430c6b71a1d059ec34d508f2710.year, player_seasonlogs.year, EXTRACT(YEAR FROM TO_TIMESTAMP(t95d728bf530769e3ed5859fbedf01162.d))), "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 5 DESC NULLS LAST, "player"."pid" asc limit 500`
    expect(query.toString()).to.equal(expected_query)
  })
  ...
  it('year split with multiple rate type with statements', () => {
    ...
    const expected_query = `with "t27683ea89d13a0a32dc2f007aba6f900" as (select "pid", count(*) as "rate_type_total_count", "year" from "player_gamelogs" left join "nfl_games" on "nfl_games"."esbid" = "player_gamelogs"."esbid" where "seas_type" = 'REG' and "year" in (2020, 2021, 2022, 2023) and "week" in (1, 2, 3, 4, 5, 6, 7, 8) group by "year", "pid"), "tc1edbd6669f91debeacfb8ba9b9bc727" as (select "pid", count(*) as "rate_type_total_count", "year" from "player_gamelogs" left join "nfl_games" on "nfl_games"."esbid" = "player_gamelogs"."esbid" where "seas_type" = 'REG' and "year" in (2020, 2021, 2022, 2023) and "week" in (10, 11, 12, 13, 14, 15, 16, 17, 9) group by "year", "pid"), "t6d5b89bed98f8824f365814749277fb4" as (select fantasy_points_plays.pid, ROUND(SUM(CASE WHEN pid_type = 'trg' AND comp = true THEN 1 ELSE 0 END + CASE WHEN pid_type IN ('bc', 'trg') THEN COALESCE(rush_yds, 0) + COALESCE(recv_yds, 0) ELSE 0 END * 0.1 + CASE WHEN pid_type = 'psr' THEN COALESCE(pass_yds, 0) ELSE 0 END * 0.04 + CASE WHEN pid_type = 'bc' AND rush_td = true THEN 6 WHEN pid_type = 'trg' AND pass_td = true THEN 6 WHEN pid_type = 'psr' AND pass_td = true THEN 4 ELSE 0 END + CASE WHEN pid_type = 'psr' AND int = true THEN -1 ELSE 0 END + CASE WHEN pid_type = 'fuml' THEN -1 ELSE 0 END), 2) as fantasy_points_from_plays, "fantasy_points_plays"."year" from (select bc_pid as pid, 'bc' as pid_type, rush_yds, recv_yds, rush_td, td, comp, int, pass_yds, pass_td, play_type, seas_type, year, week from "nfl_plays" where "bc_pid" is not null union all select psr_pid as pid, 'psr' as pid_type, rush_yds, recv_yds, rush_td, td, comp, int, pass_yds, pass_td, play_type, seas_type, year, week from "nfl_plays" where "psr_pid" is not null union all select trg_pid as pid, 'trg' as pid_type, rush_yds, recv_yds, rush_td, td, comp, int, pass_yds, pass_td, play_type, seas_type, year, week from "nfl_plays" where "trg_pid" is not null union all select player_fuml_pid as pid, 'fuml' as pid_type, NULL as rush_yds, NULL as recv_yds, NULL as rush_td, NULL as td, NULL as comp, NULL as int, NULL as pass_yds, NULL as pass_td, play_type, seas_type, year, week from "nfl_plays" where "player_fuml_pid" is not null) as "fantasy_points_plays" where not "play_type" = 'NOPL' and "fantasy_points_plays"."seas_type" = 'REG' and "fantasy_points_plays"."week" in (1, 2, 3, 4, 5, 6, 7, 8) and "fantasy_points_plays"."year" in (2020, 2021, 2022, 2023) group by "fantasy_points_plays"."year", "fantasy_points_plays"."pid"), "tf9a17cb045e9b89a056de4f5e8da67b2" as (select fantasy_points_plays.pid, ROUND(SUM(CASE WHEN pid_type = 'trg' AND comp = true THEN 1 ELSE 0 END + CASE WHEN pid_type IN ('bc', 'trg') THEN COALESCE(rush_yds, 0) + COALESCE(recv_yds, 0) ELSE 0 END * 0.1 + CASE WHEN pid_type = 'psr' THEN COALESCE(pass_yds, 0) ELSE 0 END * 0.04 + CASE WHEN pid_type = 'bc' AND rush_td = true THEN 6 WHEN pid_type = 'trg' AND pass_td = true THEN 6 WHEN pid_type = 'psr' AND pass_td = true THEN 4 ELSE 0 END + CASE WHEN pid_type = 'psr' AND int = true THEN -1 ELSE 0 END + CASE WHEN pid_type = 'fuml' THEN -1 ELSE 0 END), 2) as fantasy_points_from_plays, "fantasy_points_plays"."year" from (select bc_pid as pid, 'bc' as pid_type, rush_yds, recv_yds, rush_td, td, comp, int, pass_yds, pass_td, play_type, seas_type, year, week from "nfl_plays" where "bc_pid" is not null union all select psr_pid as pid, 'psr' as pid_type, rush_yds, recv_yds, rush_td, td, comp, int, pass_yds, pass_td, play_type, seas_type, year, week from "nfl_plays" where "psr_pid" is not null union all select trg_pid as pid, 'trg' as pid_type, rush_yds, recv_yds, rush_td, td, comp, int, pass_yds, pass_td, play_type, seas_type, year, week from "nfl_plays" where "trg_pid" is not null union all select player_fuml_pid as pid, 'fuml' as pid_type, NULL as rush_yds, NULL as recv_yds, NULL as rush_td, NULL as td, NULL as comp, NULL as int, NULL as pass_yds, NULL as pass_td, play_type, seas_type, year, week from "nfl_plays" where "player_fuml_pid" is not null) as "fantasy_points_plays" where not "play_type" = 'NOPL' and "fantasy_points_plays"."seas_type" = 'REG' and "fantasy_points_plays"."week" in (10, 11, 12, 13, 14, 15, 16, 17, 9) and "fantasy_points_plays"."year" in (2020, 2021, 2022, 2023) group by "fantasy_points_plays"."year", "fantasy_points_plays"."pid") select "player"."pid", player.fname, player.lname, "t154eac9f9dbf17872b2f79e5a75b67cc"."points_per_game" AS "points_per_game_from_seasonlogs_0", CAST(t6d5b89bed98f8824f365814749277fb4.fantasy_points_from_plays AS DECIMAL) / NULLIF(CAST(t27683ea89d13a0a32dc2f007aba6f900.rate_type_total_count AS DECIMAL), 0) AS "fantasy_points_from_plays_0", CAST(tf9a17cb045e9b89a056de4f5e8da67b2.fantasy_points_from_plays AS DECIMAL) / NULLIF(CAST(tc1edbd6669f91debeacfb8ba9b9bc727.rate_type_total_count AS DECIMAL), 0) AS "fantasy_points_from_plays_1", COALESCE(t154eac9f9dbf17872b2f79e5a75b67cc.year, t6d5b89bed98f8824f365814749277fb4.year, tf9a17cb045e9b89a056de4f5e8da67b2.year) AS year, "player"."pos" from "player" left join "t27683ea89d13a0a32dc2f007aba6f900" on "t27683ea89d13a0a32dc2f007aba6f900"."pid" = "player"."pid" left join "tc1edbd6669f91debeacfb8ba9b9bc727" on "tc1edbd6669f91debeacfb8ba9b9bc727"."pid" = "player"."pid" and "tc1edbd6669f91debeacfb8ba9b9bc727"."year" = "t27683ea89d13a0a32dc2f007aba6f900"."year" left join "scoring_format_player_seasonlogs" as "t154eac9f9dbf17872b2f79e5a75b67cc" on "t154eac9f9dbf17872b2f79e5a75b67cc"."pid" = "player"."pid" and t154eac9f9dbf17872b2f79e5a75b67cc.year = tc1edbd6669f91debeacfb8ba9b9bc727.year and t154eac9f9dbf17872b2f79e5a75b67cc.scoring_format_hash = '0df3e49bb29d3dbbeb7e9479b9e77f2688c0521df4e147cd9035f042680ba13d' left join "t6d5b89bed98f8824f365814749277fb4" on "t6d5b89bed98f8824f365814749277fb4"."pid" = "player"."pid" and t6d5b89bed98f8824f365814749277fb4.year = tc1edbd6669f91debeacfb8ba9b9bc727.year left join "tf9a17cb045e9b89a056de4f5e8da67b2" on "tf9a17cb045e9b89a056de4f5e8da67b2"."pid" = "player"."pid" and tf9a17cb045e9b89a056de4f5e8da67b2.year = tc1edbd6669f91debeacfb8ba9b9bc727.year where player.pos IN ('WR') group by player.fname, player.lname, "t154eac9f9dbf17872b2f79e5a75b67cc"."points_per_game", "t6d5b89bed98f8824f365814749277fb4"."fantasy_points_from_plays", t27683ea89d13a0a32dc2f007aba6f900.rate_type_total_count, "tf9a17cb045e9b89a056de4f5e8da67b2"."fantasy_points_from_plays", tc1edbd6669f91debeacfb8ba9b9bc727.rate_type_total_count, COALESCE(t154eac9f9dbf17872b2f79e5a75b67cc.year, t6d5b89bed98f8824f365814749277fb4.year, tf9a17cb045e9b89a056de4f5e8da67b2.year), "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 4 DESC NULLS LAST, "player"."pid" asc limit 500`
    expect(query.toString()).to.equal(expected_query)
  })
  ...
  it('keeptradecut and fantasy points from plays with year and week split', () => {
    ...
    const expected_query = `with "t0437e04c5f293392ee7470cf251ab7ff" as (select fantasy_points_plays.pid, ROUND(SUM(CASE WHEN pid_type = 'trg' AND comp = true THEN 1 ELSE 0 END + CASE WHEN pid_type IN ('bc', 'trg') THEN COALESCE(rush_yds, 0) + COALESCE(recv_yds, 0) ELSE 0 END * 0.1 + CASE WHEN pid_type = 'psr' THEN COALESCE(pass_yds, 0) ELSE 0 END * 0.04 + CASE WHEN pid_type = 'bc' AND rush_td = true THEN 6 WHEN pid_type = 'trg' AND pass_td = true THEN 6 WHEN pid_type = 'psr' AND pass_td = true THEN 4 ELSE 0 END + CASE WHEN pid_type = 'psr' AND int = true THEN -1 ELSE 0 END + CASE WHEN pid_type = 'fuml' THEN -1 ELSE 0 END), 2) as fantasy_points_from_plays, "fantasy_points_plays"."week", "fantasy_points_plays"."year" from (select bc_pid as pid, 'bc' as pid_type, rush_yds, recv_yds, rush_td, td, comp, int, pass_yds, pass_td, play_type, seas_type, year, week from "nfl_plays" where "bc_pid" is not null union all select psr_pid as pid, 'psr' as pid_type, rush_yds, recv_yds, rush_td, td, comp, int, pass_yds, pass_td, play_type, seas_type, year, week from "nfl_plays" where "psr_pid" is not null union all select trg_pid as pid, 'trg' as pid_type, rush_yds, recv_yds, rush_td, td, comp, int, pass_yds, pass_td, play_type, seas_type, year, week from "nfl_plays" where "trg_pid" is not null union all select player_fuml_pid as pid, 'fuml' as pid_type, NULL as rush_yds, NULL as recv_yds, NULL as rush_td, NULL as td, NULL as comp, NULL as int, NULL as pass_yds, NULL as pass_td, play_type, seas_type, year, week from "nfl_plays" where "player_fuml_pid" is not null) as "fantasy_points_plays" where not "play_type" = 'NOPL' and "fantasy_points_plays"."seas_type" = 'REG' group by "fantasy_points_plays"."week", "fantasy_points_plays"."year", "fantasy_points_plays"."pid") select "player"."pid", player.fname, player.lname, "tf2c4b095a714eac7d86ea8780f70ad1a"."v" AS "player_keeptradecut_value_0", "t0437e04c5f293392ee7470cf251ab7ff"."fantasy_points_from_plays" AS "fantasy_points_from_plays_0", CASE WHEN player.dob IS NULL OR player.dob = '0000-00-00' THEN NULL ELSE ROUND(EXTRACT(YEAR FROM AGE(opening_days.opening_day, TO_DATE(player.dob, 'YYYY-MM-DD'))) + (EXTRACT(MONTH FROM AGE(opening_days.opening_day, TO_DATE(player.dob, 'YYYY-MM-DD'))) / 12.0) + (EXTRACT(DAY FROM AGE(opening_days.opening_day, TO_DATE(player.dob, 'YYYY-MM-DD'))) / 365.25), 2) END as age_0, COALESCE(nfl_year_week_timestamp.year, t0437e04c5f293392ee7470cf251ab7ff.year) AS year, COALESCE(nfl_year_week_timestamp.week, t0437e04c5f293392ee7470cf251ab7ff.week) AS week, "player"."pos" from "player" left join "opening_days" on true left join "nfl_year_week_timestamp" on "nfl_year_week_timestamp"."year" = "opening_days"."year" inner join "keeptradecut_rankings" as "tf2c4b095a714eac7d86ea8780f70ad1a" on "tf2c4b095a714eac7d86ea8780f70ad1a"."pid" = "player"."pid" and "tf2c4b095a714eac7d86ea8780f70ad1a"."qb" = 2 and "tf2c4b095a714eac7d86ea8780f70ad1a"."type" = 1 and "tf2c4b095a714eac7d86ea8780f70ad1a"."d" = "nfl_year_week_timestamp"."week_timestamp" left join "t0437e04c5f293392ee7470cf251ab7ff" on "t0437e04c5f293392ee7470cf251ab7ff"."pid" = "player"."pid" and t0437e04c5f293392ee7470cf251ab7ff.year = nfl_year_week_timestamp.year and t0437e04c5f293392ee7470cf251ab7ff.week = nfl_year_week_timestamp.week where player.pos IN ('WR') group by player.fname, player.lname, "tf2c4b095a714eac7d86ea8780f70ad1a"."v", "t0437e04c5f293392ee7470cf251ab7ff"."fantasy_points_from_plays", player.dob, opening_days.opening_day, COALESCE(nfl_year_week_timestamp.year, t0437e04c5f293392ee7470cf251ab7ff.year), COALESCE(nfl_year_week_timestamp.week, t0437e04c5f293392ee7470cf251ab7ff.week), "player"."pid", "player"."lname", "player"."fname", "player"."pos" order by 4 DESC NULLS LAST, "player"."pid" asc limit 500`
    expect(query.toString()).to.equal(expected_query)
  })

...
describe('API /teams - cutlist', function () {
  ...
  describe('post', function () {
    ...
    it('one player', async () => {
      ...
      const players = await knex('league_cutlist').orderBy('sort_order', 'asc')
      expect(players.length).to.equal(1)
      expect(players[0].pid).to.equal(player.pid)
      expect(players[0].tid).to.equal(teamId)
      expect(players[0].sort_order).to.equal(0)
    })

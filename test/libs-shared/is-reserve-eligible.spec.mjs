...
describe('LIBS-SHARED isReserveEligible', function () {
  describe('backward compatibility - original logic', function () {
    it('should return true for non-ACTIVE roster_status', function () {
    ...
    it('should return true for OUT game_designation', function () {
      const result = isReserveEligible({
    ...
    it('should return true for DOUBTFUL game_designation', function () {
      const result = isReserveEligible({
        roster_status: player_nfl_status.ACTIVE,
  ...
  describe('enhanced logic - historical grace period', function () {
    ...
    describe('after final practice day - uses original eligibility logic', function () {
      ...
      it('should return false for prior week inactive + cleared injury_status', function () {
        ...
        // game_designation is null and roster_status is ACTIVE, so not eligible
        expect(result).to.equal(false)
      })

      it('should use original logic after final practice day with QUESTIONABLE game_designation', function () {
        const result = isReserveEligible({
          roster_status: player_nfl_status.ACTIVE,
          game_designation: player_nfl_injury_status.QUESTIONABLE,
          prior_week_inactive: true,
      ...
      it('should return true for prior week inactive + OUT game_designation', function () {
        const result = isReserveEligible({
          roster_status: player_nfl_status.ACTIVE,
          game_designation: player_nfl_injury_status.OUT,
          prior_week_inactive: true,
        ...
        // Friday (5) >= Friday (5), falls through to original logic
        // game_designation is OUT, so eligible
        expect(result).to.equal(true)
      })

      it('should return true for prior week inactive + DOUBTFUL game_designation', function () {
        const result = isReserveEligible({
          roster_status: player_nfl_status.ACTIVE,
          game_designation: player_nfl_injury_status.DOUBTFUL,
          prior_week_inactive: true,
        ...
        // Friday (5) >= Friday (5), falls through to original logic
        // game_designation is DOUBTFUL, so eligible
        expect(result).to.equal(true)
      })
    })
  ...
  describe('practice status eligibility', function () {
    ...
    it('should return true for DNP regardless of QUESTIONABLE game_designation', function () {
      const result = isReserveEligible({
        roster_status: player_nfl_status.ACTIVE,
        game_designation: player_nfl_injury_status.QUESTIONABLE,
        practice: {
    ...
    it('should return true for LP with ACTIVE roster_status', function () {
      const result = isReserveEligible({
        roster_status: player_nfl_status.ACTIVE,
        game_designation: null,
        practice: {
    ...
    it('should use other eligibility logic when practice status is FULL', function () {
      ...
      // FULL doesn't make eligible via practice, but OUT game_designation does
      expect(result).to.equal(true)
    })
    ...
    it('should use other eligibility logic when practice is null', function () {
      ...
      // No practice data, but roster_status is not ACTIVE
      expect(result).to.equal(true)
    })
  ...
  describe('final practice report detection', function () {
    ...
    it('should return true when game_designation field exists', function () {
      const result = isReserveEligible({
        roster_status: player_nfl_status.ACTIVE,
        game_designation: null,
        practice: {
      ...
      // Final report exists (game_designation field), so practice status check is skipped
      // Falls through to original eligibility logic
      expect(result).to.equal(false)
    })
    ...
    it('should return false when no conditions met', function () {
      ...
      // Tuesday (2) < Friday (5), no source_status/game_designation fields, final day has no status
  ...
  describe('practice status with final report', function () {
    ...
    it('should NOT make DNP eligible when final report exists', function () {
      ...
      // Falls through to original eligibility logic (game_designation is null)
      expect(result).to.equal(false)
    })
    ...
    it('should NOT make LP eligible when final report exists', function () {
      ...
      // Final report exists (game_designation field), practice status check is skipped
      // Falls through to original eligibility logic (game_designation is null)
      expect(result).to.equal(false)
    })
    ...
    it('should fall through to original logic after final report', function () {
      ...
      // Final report exists (status field), practice status check is skipped
      // Falls through to original eligibility logic
      // QUESTIONABLE game_designation is not eligible unless current_season.week === 0
      const expected = Boolean(current_season.week === 0)
      expect(result).to.equal(expected)
    })

    it('should make OUT game_designation eligible even with final report', function () {
      const result = isReserveEligible({
        roster_status: player_nfl_status.ACTIVE,
        game_designation: player_nfl_injury_status.OUT,
        practice: {
      ...
      // Final report exists (status field), practice status check is skipped
      // Falls through to original eligibility logic
      // OUT game_designation makes eligible
      expect(result).to.equal(true)
    })

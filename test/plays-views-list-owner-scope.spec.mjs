...
// GET /api/plays/views is mounted before the blanket auth guard in
// api/index.mjs, so it must self-enforce. It once had no auth check and no
// mandatory filter, which made every saved plays view on the platform
// enumerable by an anonymous caller (the same shape 216c1a5d0 fixed on
// GET /api/data-views). It is now owner-scoped with no filter parameters.
// Fetch-by-id and export stay unauthenticated on purpose: that is how a
// shared view resolves for a logged-out visitor.
describe('GET /api/plays/views is owner-scoped', function () {
  this.timeout(20000)
  let token1
  let token2
  let user1_id
  ...
  before(async function () {
    ...
    await knex('user_plays_views').insert([
      {
  ...
  it('rejects an unauthenticated caller', async function () {
    const res = await chai_request.execute(server).get('/api/plays/views')
  ...
  it('ignores a user_id parameter naming another user', async function () {
    const res = await chai_request
      .execute(server)
      .get(`/api/plays/views?user_id=${user2_id}`)
      .set('Authorization', `Bearer ${token1}`)
  ...
  it('ignores a username parameter naming another user', async function () {
    const res = await chai_request
      .execute(server)
      .get('/api/plays/views?username=scopetest2')
      .set('Authorization', `Bearer ${token1}`)
  ...
  it('still owner-scopes the list for a second user', async function () {
    const res = await chai_request
      .execute(server)
      .get('/api/plays/views')
  ...
  it('still resolves another user view by id without authentication', async function () {
    const res = await chai_request
      .execute(server)
      .get('/api/plays/views/3c2b8a4d-7e59-4f10-8a6c-1d9b4e7f0a35')

    res.should.have.status(200)
    res.body.view_name.should.equal('Owned By Two')
    res.body.user_id.should.equal(user2_id)
  })
})

export default {
  ...
  player_opportunities_from_plays:
    'The number of opportunities by the player (pass attempts excluding sacks + rush attempts + targets, calculated from play-by-play data)',
  ...
  team_pass_rate_over_expected_from_plays:
    'Team Pass Rate Over Expected (PROE) is the difference between the actual pass rate and the expected pass rate generated by the nflverse model which accounts for play context: yard line, whether possession team is at home, roof type (retractable, dome, or outdoors), down, quarter, half seconds remaining, yards to go, score differential, timeouts remaining for each team, and win probability (both with spread and non-spread adjusted). PROE can indicate team tendencies and passing aggressiveness.',
  ...
  // min/max (MFL, CBS), sample_size (MFL), percent_drafted (ESPN/Yahoo/MFL/CBS)
  // are populated only by the secondary sources; null for the live Sleeper /
  // Underdog feeds. player_adp_stddev removed 2026-06-29 (no source provides it).

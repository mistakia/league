 ...
 * in the SPA -- hence `Date | string` rather than the row type's bare `Date`.
 * dayjs() accepts both; dayjs.unix() accepts neither and would have yielded an
 * Invalid Date with nothing raising.
 *
 * @param {object} season
 * @param {Date | string | null} [season.free_agency_period_start]
 * @param {Date | string | null} [season.free_agency_period_end]
 * @param {Date | string | null} [season.free_agency_live_auction_start]
 * @param {Date | string | null} [season.free_agency_live_auction_end]
 */
export default function get_free_agent_period({
  ...
  return {
    ...
    free_agency_live_auction_end: free_agency_live_auction_end_dayjs,
      ...
      : current_season.regular_season_start.add(1, 'week').subtract(1, 'minute')
  }
}

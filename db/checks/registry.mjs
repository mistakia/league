...
import { prop_market_rung_duplication_rows } from '#libs-server/prop-market-rung-duplication.mjs'
...
import { nfl_play_stats_game_linkage_rows } from '#libs-server/nfl-play-stats-game-linkage.mjs'
import {
...
const registry = [
...

  {
    ...
    min_rate: 1.0,
    calibration:
      "EXACT, not a tolerance: a FINAL game either has plays or it does not, so the healthy reading is 1.0 and min_rate 1.0 is a live invariant. THE SEASON FLOOR IS GONE, and the reason it existed is worth keeping because it is a lesson about what a calibration can get wrong. It used to read `season_year >= 2015`, justified by 2014 PRE weeks 1-4 sitting at 0.0000 and by the nfl-plays-games-season-agreement calibration calling that an era where the feed predated our coverage. THAT WAS FALSE. The plays existed the whole time, filed under a forked esbid no nfl_games row answered to, and db/adhoc/2026-09-03-repair-forked-play-stats-esbids.sql re-pointed them. 2014 PRE weeks 1-4 now read 1.0000. The floor was not protecting the check from an un-closeable era; it was hiding a repairable defect, and an inherited explanation was what kept anyone from looking. 2013 PRESEASON IS NOW GRADED, and the mechanism that had excluded it is worth keeping because it is a second way a season goes silent here. All 65 of its games carried a NULL status, so `status like 'FINAL%'` excluded them while their plays sat present and correct — absence by a missing stamp rather than by missing plays, which reads identically from this check. db/adhoc/2026-09-03-stamp-2013-preseason-final.sql stamped them FINAL and weeks 1-4 now read 16 of 16; PRE week 0 is one game and stays un-gradeable under the eight-game precondition. The inherited claim that 2013 was the only unstamped season between 2002 and 2025 was NOT correct and should not be re-copied: 2017, 2018 and 2019 each carry a few unstamped PRE games, 2020 a PRO, and 2022 one POST and one REG. 2013 PRE was the only whole season-type block, which is why it alone cost a season of coverage. MEASURED 2026-09-03 WITH NO FLOOR, which is what made removing it safe rather than a widening: the whole 2002-onward corpus holds 499 gradeable weeks against the 230 the floor allowed, and exactly ONE reads short — 2021 PRE week 3 at 15 of 16, already PARKED as adjudicated, because NFL Pro returns no plays at all for 2021082855 (measured 2026-08-26 on a live premium token) so there is nothing to import. Dropping the floor therefore more than doubles what is graded and introduces zero new findings. THE PRO BOWL NEEDED NO SPECIAL CASE, which is the other thing that made this clean: it is one game a week and reads 0.0000 every year, but the eight-game precondition already reports it un-gradeable rather than as a finding, so extending the range backwards does not drag it in. The other defect this was written for, 2025 PRE week 3 at 0.0625, was repaired earlier. Both real defects have been preseason, which is the season_type every scheduled path treats as optional.",
    // 499 gradeable weeks today against the 230 the old 2015 floor allowed, and
    // the corpus grows by about 23 a season, so the count cannot fall except by
    // a scan collapsing. 400 sits under today's figure at the same margin the
    // previous 180-against-229 floor held.
    min_gradeable_units: 400,
    repair_command:
  ...
  {
    check_id: 'prop-market-rung-duplication',
    invariant:
      "One priced rung of one market -- one book, one game, one market type, one runner, one line, one OPEN-or-CLOSE observation -- is carried under exactly one `source_market_id`. `prop_markets_index` is unique on (source_id, source_market_id, time_type), and that middle column is the BOOK'S handle rather than an identity we control, so a book issuing a second id for a market it is already offering leaves both keyings alive as concurrent observations of one real rung, disagreeing on price. Nothing else here sees it: the importers report per-run success for runs that happened, both keyings are individually well-formed, and prop-market-selection-coverage reads HEALTHIER when this fires, because the duplicate market carries its own selections and so counts as covered. Found 2026-09-03 through a data view rendering one ladder rung twice at two prices; neither 2025 incident raised anything anywhere.",
    grain: ['source_id', 'season_year'],
    rows: prop_market_rung_duplication_rows,
    // A book-season too thin to judge is not evidence of health. 1,000 rung
    // keys is a few games of one book's ladders, well under any real season
    // and above the handful a newly-added or just-retired book leaves.
    precondition: (/** @type {Record<string, any>} */ row) =>
      row.denominator >= 1000,
    min_rate: 0.97,
    calibration:
      "THE POPULATION IS THE CALIBRATION HERE, ahead of the threshold, because the first two measurements of this defect were both wrong in the same direction and for the same reason. SQL's `group by` treats NULL as equal to NULL, so a null grain column does not weaken the rung key, it MERGES rows the key exists to separate, and every merged pair then reports as a duplicate. Both traps are live in this corpus, measured 2026-09-03: `market_type` is null on 77 percent of DRAFTKINGS markets and 94 percent of PINNACLE's, and grouping those together reported 'Alt Spread 1st Quarter' and 'Alt Spread 1st Half' as one duplicated rung, inflating DRAFTKINGS 2025 from 15,249 duplicated keys to 88,513; `selection_metric_line` is null on 232,137 of CAESARS 2024's 232,151 alt selection rows, collapsing that book-season's whole ladders to one key each and manufacturing 40,628 more. Excluding both nulls moves the corpus-wide total from about 244,000 duplicated keys to 58,611, and it is the smaller figure that is this check's subject. THE THRESHOLD, measured over that population across 19 (source_id, season_year) pairs. EIGHT read EXACTLY 1.0000 -- FANDUEL 2023 and 2024, CAESARS 2023, 2025 and 2026, PINNACLE 2024 and 2026, BETMGM 2026 -- so a clean book-season is not merely near-perfect, it is perfect, and there is no background duplication rate this check is obliged to tolerate. Every reading under 1.0 was run to cause and every one is real: PINNACLE 2025 0.5711, DRAFTKINGS 2023 0.8675, DRAFTKINGS 2025 0.9483, PRIZEPICKS 2026 0.9815, FANDUEL 2025 0.9856, PRIZEPICKS 2023 0.9860, PRIZEPICKS 2024 0.9902, then a cluster at 0.9954-0.9956 (CAESARS 2024, DRAFTKINGS 2024, DRAFTKINGS 2026, PRIZEPICKS 2025). 0.97 sits in the widest gap in that sequence, between 0.9483 and 0.9815. IT IS DELIBERATELY LOOSE AND DOES NOT CLAIM THE CORPUS IS CLEAN: it is a floor on the INCIDENT shape -- a book re-keying at season scale, which is what DRAFTKINGS 2025 and PINNACLE 2025 are -- and it knowingly passes eight book-seasons carrying real low-grade duplication, because a floor set at the honest 1.0 would fire on eleven of nineteen pairs at once and say nothing about which of them moved. Tightening it toward 1.0 is the right direction as the classes below are repaired, and each tightening should be taken deliberately rather than by drift. THE DUPLICATES PARTITION INTO THREE SHAPES and the split decides who owns each, measured over the same population: 40,030 of the 58,611 are a market-id re-key under a STABLE source_event_id, which is the shape the supersession rule addresses; 14,264 are CONCURRENT -- two ids at the identical observed_at, where no later-supersedes-earlier rule is even defined, and PINNACLE 2025 is 3,565 of them; and 4,317 are a new source_event_id for a game already covered, which is most prevalent in books this defect was not first noticed in (PRIZEPICKS carries 185 such games in 2024 against FANDUEL's 1 in 2025). WHAT THIS CANNOT SEE: a rung whose market_type or selection_metric_line never parsed is outside the population entirely, so a book that re-keys a market whose line was never captured is invisible here and only the line defect itself will surface it; and a book that encodes the line INTO its market id, as PINNACLE does (`1614603339/s;0;s;5.5`), is correctly not penalised for it, because selection_metric_line is a grain column and factors the encoding out -- PINNACLE's 0.5711 is residual duplication that survives that factoring, not the encoding showing through.",
    // 16 of today's 19 book-season pairs clear the precondition, and the count
    // grows by roughly one per book per season. 12 sits under that and far
    // above the reading it exists to catch, which is the group-by collapsing
    // and reporting a handful of books.
    min_gradeable_units: 12,
    // The smallest graded population today is PINNACLE 2024 at 13,928 rung
    // keys. 5,000 sits under that and orders of magnitude above a scan that
    // reached almost nothing.
    min_denominator: 5000,
    repair_command:
      "Classify the book-season before repairing anything, because the three shapes have different owners and one of them has no repair rule at all. Run the rung grain with a per-key `count(distinct source_market_id)`, `count(distinct observed_at)` and `count(distinct source_event_id)` and split on the last two: equal observed_at across the ids is a CONCURRENT duplicate, where the book served one rung twice in one snapshot and neither keying supersedes the other -- do not apply a newest-wins rule to these, it picks arbitrarily. More than one source_event_id is the book re-keying the GAME, which usually means a schedule change or a postponement and is worth confirming against nfl_games before touching. One event id and distinct times is a market-id re-key, the only shape a later-supersedes-earlier rule fits. THEN CHECK THE OPEN ROWS SEPARATELY, because they carry a second defect the duplicate count does not show: `build_market_index_merge` in libs-server/insert-prop-markets.mjs makes observed_at first-write-wins on the OPEN row keyed by source_market_id, so a re-key restarts that clock and stamps a game-day price as an opening line. Do NOT repair those by carrying the earliest observed_at forward onto the later row -- the later row's PRICE is a game-day price, and moving only the timestamp onto it makes the record worse rather than better by making a late price look early. The recoverable repair is to prefer the earliest keying's row wholesale and retire the later one; prop_markets_history carries neither market_type nor time_type and so cannot arbitrate this on its own."
  },

  {
    ...
    grain: ['feed', 'partition_column', 'partition_value'],
    rows: index_parity_rows,
    max_count: 0,
    calibration:
      "THE GRAIN IS THE PRODUCER, NOT THE FEED, and that is the whole calibration: a feed-grained finding cannot carry a disposition, because two of these feeds hold two unrelated causes at once and one entry can only say one of them truthfully. projections is the case that forced it -- 108,215 of its 130,825 unbacked rows are a deliberate design decision and 22,610 are a real gap, and no honest entry covers both. The reported key is the feed's OWN producer column rather than one column name for everything: source_id on the ten feeds that carry it, league_format_id on the two league-format ones, and null on the four that carry neither, which report whole exactly as they did before. Both halves of that key are reported (partition_column, partition_value), so a parked entry naming league_format_id goes STALE rather than silently continuing to suppress if the feed ever changes which column names its writer. Partitioning is a decomposition and not a different comparison -- the producer column is always already a grain column -- and the per-producer counts were checked against the whole-feed counts they replace: projections sums to exactly 130,825 over the 13 of its 16 producers that diverge, and season_projections to exactly 6,801 over the 6 of its 10, which are the same two totals the feed-grained run reported. MEASURED 2026-09-03 against production, after the 2026-09-02 SLEEPER delete (db/adhoc/2026-09-02-remove-orphaned-sleeper-rankings-rows.sql): 14 derived pairs decompose into 112 rows, 108 gradeable and 4 un-gradeable, of which 83 are exact within coverage and 25 report as findings. THE FINDINGS, largest first: projections source 18 at 108,215 of 108,215 scanned and its twelve other sources at 134 to 5,159 each; league_format_player_projection_values at 45,485 of 79,955 (07db3422), 12,223 of 46,423 (f663e70c) and 11,556 of 11,556 (776e3d91); season_projections source 18 at 6,787 of 6,787 and five other sources at 1 to 6 rows each; player_rankings FANTASYPROS at 1,571 of 59,436, down from 109,103 before the SLEEPER delete and now the feed's only producer; league_format_player_rest_of_season_projection_values at 642 of 642 (776e3d91); prop_market_selections FANDUEL at 435 of 1,386,099. Every one of the 25 is now dispositioned in db/checks/parked.json except player_rankings FANTASYPROS, which is deliberately left unparked and split into its own task. RE-MEASURE ANY FIGURE ON THE LEAGUE-FORMAT FEEDS BEFORE ACTING ON IT: they are under active write, and that feed's index total moved 409,869 -> 407,889 inside a few minutes on 2026-09-02. FOUR pairs are un-gradeable every run and say so rather than passing: nfl_draft_rankings (its _index carries no unique key, so the grain is underivable, and it holds zero rows besides), props (source_id is an enum on one side and an integer on the other, which raises 42883 rather than answering), and the two declared unpaired halves espn_receiving_metrics_history and historical_injury_index. WHAT THIS CANNOT SEE: the comparison is over the row SET, so a snapshot holding a STALE value at a correct grain key passes; coverage is scoped to the (season_year, week, season_type) tuples present in history, so a feed whose history began mid-life is judged only inside its own window; and a producer that vanishes from an index leaves no row at all rather than an empty one, so it is visible in the unit count and nowhere else. A producer present only in HISTORY is deliberately absent from the report -- equality is claimed in the index-to-history direction alone, so it has no rows to grade.",
    // 108 graded producers over 14 pairs today, every one of them derived: the
    // pairs from information_schema and the producers from the index halves
    // themselves. 80 sits under today's count with room for a feed to retire,
    // and far above the reading it exists to catch, which is the derivation
    // returning nothing and reporting a clean run over zero producers.
    //
    // This floor carries the detector health alone, and NO `min_denominator`
    // is declared, which is a deliberate change from the feed-grained form.
    // That rule binds where the row count is fixed by construction; it is not
    // fixed here, because the row set is one per producer PRESENT in the index
    // rather than one per pair. A per-producer floor would also fire on the
    // wrong thing: the smallest graded population is now a small producer
    // (selection_combination_odds FANDUEL at 81 rows), so any floor above it
    // reports a legitimately thin book as a tooling failure -- calibrating on
    // the worst normal reading, which this registry refuses. What is lost with
    // it is narrow and worth naming: a partial scan collapse confined to ONE
    // small producer no longer trips a floor.
    min_gradeable_units: 80,
    repair_command:
      'Establish first whether the INDEX gained rows or the HISTORY lost them; they have opposite owners and the count alone cannot tell you. The finding names ONE producer of one feed, so read it against that producer rather than the feed: the same feed routinely holds a correct-by-design divergence under one source and a real gap under another. Compare the feed\'s writer against its history writer -- a snapshot written by a path that does not also append history produces exactly this, and so does a history retention job. Do NOT "repair" by deleting index rows: for a feed whose history began mid-life the index is the older and more complete side. Re-measure with node -e "import(\'#libs-server/population-levels.mjs\').then(m => m.index_parity_rows().then(console.log))" under NODE_ENV=production -- ONCE. It scans every feed\'s full history and holds a 5-minute cooldown lock; re-running it in a wait-and-recheck loop is what put 90-208s of load on production every few minutes for five hours on 2026-09-02. Re-measure after you change something, not while waiting to see if something changes.'
  },
...

  {
    ...
    min_gradeable_units: 400,
    repair_command:
      'THE HISTORICAL DEBT IS ALREADY REPAIRED, so a finding here is almost certainly a REGRESSION rather than more of the same backlog -- an importer has reintroduced an era token, and the fix belongs in that importer, not in the data. Find the writer before touching a row. Do NOT hand-edit tokens to clear a finding, and do NOT change libs-shared/fix-team.mjs: it has 350 call sites across 71 files and is a pure function of the token with no season parameter, so it cannot express this rule even in principle. The correct normalizer is resolve_canonical_nfl_team in libs-shared/nfl-team-franchise-eras.mjs. If the token is one this repo has never modelled, the repair is to add it there -- as an era range if its meaning moves with the season, or to nfl_team_spelling_aliases if it names one franchise across its whole history -- rather than to widen anything here. NO SLOTS ARE EXPECTED ANY MORE. The four empty-string slots that used to be carved out here were repaired on 2026-09-03 and this check now reads zero, so EVERY finding is a live defect and nothing here should be waved through. AN EMPTY STRING IS THE ONE SHAPE WORTH RECOGNISING ON SIGHT, because the repaired four proved it can mean two unrelated things -- see user:task/league/resolve-empty-string-nfl-team-slots.md. Ask which before repairing: in nfl_plays those four were genuinely empty placeholder NOPL rows with every other column null, against 88,414 siblings already carrying a NULL possession, so NULL was right; in player_gamelogs they were NOT an absence at all but Jaguars rows misattributed by scripts/generate-player-snaps.mjs, and NULL or INA would both have been wrong. player_gamelogs.nfl_team no longer carries a DEFAULT, so a writer that omits the column now raises at write time rather than minting an empty string -- an empty string reappearing there means a writer is inserting one deliberately, which is a different and louder defect than the one that produced these. If you genuinely need to undo the 2026-09-03 conform, the five preconform_20260902 tables hold the prior state, and nfl_play_stats is the full physical copy because that table has no usable row handle.'
  },
  {
    check_id: 'nfl-play-stats-game-linkage',
    invariant:
      "Every nfl_play_stats row's esbid resolves to an nfl_games row. THIS CHECK EXISTS BECAUSE THREE OTHERS DECLINED THE POPULATION AND SAID SO: nfl-team-abbreviation-conformance emits it as a zero-denominator un-gradeable row, and gamelogs-games-season-agreement and nfl-plays-games-season-agreement both name these games in their calibrations and hand them on in the same words -- a growing orphan population means games are missing and is a different condition with a different owner. Nothing was that owner, so the class sat behind a console line and a signal that fires only inside the runner's coverage-collapse branch, free to grow by an order of magnitude with nothing firing. THE PREVAILING DESCRIPTION OF THE CLASS IS WRONG AND THOSE THREE CALIBRATIONS REPEAT IT: the games are NOT missing. Measured 2026-09-03, all 130 of the 2013 and 2014 orphan esbids resolve to a real PRE game at esbid + 50, 130 for 130 -- the play-stats importer minted an esbid from a date and a sequence rather than resolving one against nfl_games, and used the regular-season 00+ sequence for two preseason slates whose games carry the 50+ convention. The defect is a FORKED KEY, not an absent game, which is why every entry here is standing debt with an owning repair rather than an adjudication: each names a game we hold, so a repair can land.",
    grain: ['esbid'],
    rows: nfl_play_stats_game_linkage_rows,
    max_count: 0,
    calibration:
      "EXACT, not a tolerance: an esbid either names a game we hold or it does not, and every orphan measured has a resolvable target, so max_count 0 is a live invariant rather than an aspiration. THE NUMERATOR IS 1 PER ORPHAN ESBID, NOT ITS ROW COUNT, deliberately: max_count is a budget over the whole unsuppressed violation count, so a numerator of 1 at a grain of esbid makes the budget count GAMES -- the unit the repair acts on and the unit a parked entry is keyed by -- rather than a six-figure row count nobody can read. THE POPULATION THIS WAS REGISTERED TO WATCH IS NOW REPAIRED AND THE CHECK READS ZERO, re-measured 2026-09-03 at 8,291 gradeable esbids with 0 findings and all 132 parked entries deleted. db/adhoc/2026-09-03-repair-forked-play-stats-esbids.sql re-pointed the 130 and deleted the two singletons. The BEFORE reading is kept below because it is what calibrates the detector, and because the 132-to-0 move is the paired reading that makes this zero a measurement rather than a query that could never speak. MEASURED ON PRODUCTION 2026-09-03, BEFORE THE REPAIR: 8,293 distinct esbids over 5.7M play-stat rows, 132 of them orphaned, carrying 85,993 rows. The 132 are four cohorts and the grain separates them: 2013 PRE (65 games, 42,974 rows), 2014 PRE (65 games, 41,745 rows), and two singletons that are the SAME mechanism meeting a moving schedule -- 2020112602 is the Thanksgiving slot of a BAL at PIT game postponed to 2020120200, and 2021122201 is a SEA at LA game nfl_games carries one calendar day earlier at 2021122101. THOSE TWO SINGLETONS ARE THE WHOLE ARGUMENT FOR REGISTERING THIS: they are not part of the historical block, they accrued afterwards, and no threshold or signal in the system moved when they did. THE TARGET MAPPING WAS PROVEN EXHAUSTIVELY RATHER THAN SAMPLED: esbid + 50 exists in nfl_games for 130 of 130 of the 2013/2014 orphans and is season_type PRE in 130 of 130; the team pair agrees outright on 114 and the 16 that do not are EXACTLY the STL and SD rows, which read as disagreements only because the 2026-09-03 abbreviation conform reached nfl_games through the very esbid join that cannot reach these rows. A mapping that survives its own exceptions being explained by an unrelated measured event is stronger evidence than the 114 agreements alone. DEMONSTRATED RED AND GREEN BY SEEDING, not by inverting the predicate: test/db.checks-nfl-play-stats-game-linkage.spec.mjs seeds a play-stat row with a game and requires the check to stay silent, then removes the game and requires it to report that esbid, so the shipped query is what the test exercises. THE DETECTOR-HEALTH FLOOR READS EMITTED ROWS, which is why this check emits one row per esbid rather than one per orphan: a violations-only shape emits nothing over a clean corpus and reads as a collapsed scan. min_gradeable_units 7,000 sits under today's 8,293 and is breached by any large loss of the table; the population grows by about 285 esbids a season, so it cannot fall except by a scan collapsing. min_denominator 200 is calibrated on the GAP rather than the worst normal reading: the smallest real game carries 326 play-stat rows and the first percentile is 435, so 200 is unreachable by a real game and reached immediately by a partial scan. THE FORK IS NOT CONFINED TO THIS TABLE, AND THAT IS WHY THERE IS ONE CHECK RATHER THAN ONE PER TABLE: nfl_plays holds 23,858 rows under those same 132 esbids, all 132 of them, and player_gamelogs a further 2,310 across 124 of them, because the whole play-import path took the forked key rather than resolving one. THAT THIRD TABLE WAS FOUND BY ENUMERATING FROM THE SCHEMA rather than from the two already known, and it is the reason to do so: of every esbid-keyed table in production only those three held orphan rows, while nfl_snaps, the four nfl_plays_* role tables, nfl_player_play_charting, play_changelog, nfl_matchup_stats, nfl_team_gamelogs, historical_injury_index and the three format gamelog tables all read exactly 0. A new fork therefore appears in both tables under the SAME esbid and is reachable from this single detector, so a per-table sibling would be one invariant with two graders that can drift and disagree about method -- the shape the snaps and gamelogs season-agreement checks each declined a sharper variant to avoid. It also corrected a third calibration, which has since been fixed rather than left as an implication: nfl-plays-game-coverage (NOT `nfl-plays-week-coverage`, a name that never existed and that this sentence used to cite) read 2014 PRE weeks 1-4 at 0.0000 and blamed a feed predating our coverage. The plays existed all along under the forked esbid, and that check's season floor has been removed accordingly. COST: 0.75 seconds against production, a grouped left join over the whole table.",
    min_gradeable_units: 7000,
    min_denominator: 200,
    repair_command:
      'A FINDING IS A CANDIDATE, NOT AN INSTRUCTION, and the wrong remediation here is unrecoverable: do NOT delete orphan rows. Every orphan measured so far names a game we DO hold under a different esbid, so the repair is to re-point ps.esbid at the real game, never to drop the rows. Identify the target before acting -- for a 2013 or 2014 PRE orphan it is esbid + 50, verified for all 130; for anything else, match on the game date within one day and the team pair resolved through resolve_canonical_nfl_team in libs-shared/nfl-team-franchise-eras.mjs, NOT on the raw token, because the abbreviation conform moved nfl_games to canonical while these rows kept their era token. Check for a collision first: exactly one of the 132 known targets already carries play stats of its own, so a blind update would merge two games. If no target resolves, that is the one case where the finding may genuinely be a missing game, and it needs a game import rather than a key repair -- 21 of the 132 did not resolve on a raw-token match, and 2020112602 is one of them only because all its team values are null. THE WRITER IS THE REAL FIX: the play-stats importer mints an esbid from a date and a sequence instead of resolving one against nfl_games, so a repair alone is a dated snapshot of a still-running producer -- the 2020 and 2021 singletons are that producer accruing after the historical block. The 132 known games are parked as baselined debt, so a finding here is a NEW orphan and means the writer has forked another key.'
  }
]

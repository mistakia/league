 ...
 *
 *   "The Candidate a Team ranks first receives points equal to the number
 *    stated in that Notice, the Candidate it ranks second one (1) fewer, and so
 *    on. A Candidate whom a Team does not rank, or ranks below the number
 *    stated in that Notice, receives no points from that Team."
 *
 ...
 * scores its first preference at the full stated maximum.
 *
 ...
 * a rank beyond the stated maximum -- checkable without a database.
 *
 * The maximum is also the reason a rank beyond it is tolerated here rather than
 * thrown on. The submit route enforces the bound, so such a row should not
 * exist; if one ever does, Section 10(b) says in terms what it is worth, and
 * scoring it at zero is that answer rather than an exception that would strand
 * a whole tally.
 *
 * @param {Object} args
 ...
 * @param {number} args.maximum_ranked_candidates - The number stated in the
 *   Section 10 Notice, from the vote row.
 ...
 *   the result at zero rather than leaving him absent, which Section 10(e)

...
export const apply_scope_to_query = ({
  ...
  //
  // This states the week restriction twice -- nfl_week_id is GENERATED from
  // (season_year, season_type, week), so the composite sits alongside the year
  // and seas_type predicates below and the planner multiplies the two
  // encodings as independent, estimating 4 against an actual 272 on nfl_games.
  // Emitting the decomposed `week IN (...)` instead fixes that estimate and was
  // shipped for a time, but measured SLOWER in both regimes and was backed out:
  // 1.043x aggregate under enable_nestloop=off across 45 statements, 1.217x
  // under the default planner across the 3 that run there. The estimate is
  // wrong and does not matter -- the nested-loop arm picks a hash join
  // regardless of it. Do not re-derive the swap from the estimate alone.
  if (
    has_nfl_week_id &&

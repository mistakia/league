...
export default class FfpcAdapter extends BaseAdapter {
  ...
  async authenticate(credentials) {
    return stub_authenticate(this.log.bind(this), 'FFPC')
  }

  async get_league(league_id, options = {}) {
    stub_get_method(this.log.bind(this), 'FFPC', 'get_league', {
      league_id,
      options
    })
  }
  ...
  async get_teams(league_id) {
    stub_get_method(this.log.bind(this), 'FFPC', 'get_teams', { league_id })
  }

  async get_rosters({ league_id, week = null, year = null }) {
    stub_get_method(this.log.bind(this), 'FFPC', 'get_rosters', {
      league_id,
      week,
      year
    })
  ...
  async get_players({ filters = {} } = {}) {
    stub_get_method(this.log.bind(this), 'FFPC', 'get_players', { filters })
  }

  async get_transactions({ league_id, options = {}, year = null }) {
    stub_get_method(this.log.bind(this), 'FFPC', 'get_transactions', {
      league_id,
      options,
      year
    })
  ...
  async get_matchups({ league_id, week }) {
    stub_get_method(this.log.bind(this), 'FFPC', 'get_matchups', {
      league_id,
      week
    })
  }
  ...
  async get_scoring_format(league_id) {
    stub_get_method(this.log.bind(this), 'FFPC', 'get_scoring_format', {
      league_id
    })
  }

  async get_draft_results(league_id) {
    stub_get_method(this.log.bind(this), 'FFPC', 'get_draft_results', {
      league_id
    })
  }

...
const generate_table_alias = ({ type, params = {} } = {}) => {
  ...
  const hash = Array.from(blake2b(`${type}_${key}`, null, 16))
...
const league_format_player_projection_values_table_alias = ({
  ...
  const key = `league_format_player_projection_values_${year}_week_${week}_${league_format_hash}`
...
const scoring_format_player_projection_points_table_alias = ({
  ...
  const key = `scoring_format_player_projection_points_${year}_week_${week}_${scoring_format_hash}`
  const hash = Array.from(blake2b(key, null, 16))
    .map((b) => b.toString(16).padStart(2, '0'))
    .join('')
  return `t${hash}`
